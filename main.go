...
func drawcam() { //MARK:DRAW CAM
  ...
  if gs.UI.OptionsOn {
    ...
    drawNextLevelScreen(&gs.Level, &gs.Core, &gs.Player, &gs.Input)
  } else if gs.Player.Died {
    drawDied(&gs.Core, &gs.Level, &gs.Render, &gs.Player, &gs.Timing)
  }
  ...
  if !gs.Core.Pause {
    ...
    for a := 0; a < len(dFloors); a++ {
      drawBlok(&gs.Render, &gs.Core,dFloors[a], false, false, 0)
    }
    ...
    if len(gs.Level.Level[gs.Level.RoomNum].spikes) > 0 {
      for a := 0; a < len(gs.Level.Level[gs.Level.RoomNum].spikes); a++ {
        ...
        if rl.CheckCollisionRecs(gs.Player.Pl.crec, gs.Level.Level[gs.Level.RoomNum].spikes[a].rec) {
          hitPL(&gs.Core, &gs.Level, &gs.Render, &gs.Audio, &gs.Player, &gs.Enemies, &gs.UI,0, 2)
        }
      }
    }
    ...
    for a := 0; a < len(dWalls); a++ {
      drawBlok(&gs.Render, &gs.Core,dWalls[a], false, false, 0)
      //WALL BLOCKNUMS
      if gs.Core.Debug {
    ...
    if len(gs.Level.Level[gs.Level.RoomNum].innerBloks) > 0 {
      for a := 0; a < len(gs.Level.Level[gs.Level.RoomNum].innerBloks); a++ {
        drawBlok(&gs.Render, &gs.Core,gs.Level.Level[gs.Level.RoomNum].innerBloks[a], false, true, 4)
      }
    }
    ...
    if gs.FX.ChainLightOn {
      drawChainLight(&gs.FX, &gs.Level, &gs.Core, &gs.Player, &gs.Audio)
    }
    ...
    for a := 0; a < len(dMovBloks); a++ {
      drawBlok(&gs.Render, &gs.Core,dMovBloks[a], true, false, 0)
    }
    ...
    if len(gs.Player.Inven) > 0 {
      drawInven(gs.Render, gs.Player, gs.Level, gs.Core.Ori)
    }
    //PLAYER INFO
    drawPlayerInfo()
...
func drawNextLevelScreen(level *LevelState, core *CoreState, player *PlayerState, input *InputState) { //MARK:DRAW NEXT LEVEL SCREEN
  txt := "prepare for level " + fmt.Sprint(level.Levelnum)
  txtlen := rl.MeasureText(txt, 40)
  x := int32(core.Cnt.X) - txtlen/2
  y := int32(core.Cnt.Y - 20)
  rl.DrawText(txt, x, y, 40, rl.White)
  txt = "press space or button to continue"
  txtlen = rl.MeasureText(txt, 20)
  x = int32(core.Cnt.X) - txtlen/2
  y = int32(core.Cnt.Y + 30)
  rl.DrawText(txt, x, y, 20, rl.White)
  if level.NextlevelT > 0 {
    level.NextlevelT--
  } else {
    player.StartdmgT = core.Fps * 5
    if rl.IsKeyPressed(rl.KeySpace) {
      level.NextLevelScreen = false
      core.Pause = false
    }
...
func drawHelp(core *CoreState, level *LevelState, ui *UIState) { //MARK:DRAW HELP
	rl.DrawRectangle(0, 0, core.ScrW32, core.ScrH32, rl.Black)
	txt := "help"
	txtlen := rl.MeasureText(txt, txU5)
	txtx := int32(core.Cnt.X) - txtlen/2
	txty := int32(level.LevY) + txU
	rl.DrawText(txt, txtx, txty, txU5, rl.White)
	txtx = int32(core.Cnt.X - bsU9)
	txty += txU7
	txt = "five levels collect power ups"
	rl.DrawText(txt, txtx, txty, ui.TxtSize, rl.White)
	txty += ui.TxtSize + txU/4
...
func drawDied(core *CoreState, level *LevelState, render *RenderState, player *PlayerState, timing *TimingState) { //MARK:DRAW DIED
  rl.DrawRectangle(0, 0, core.ScrW32, core.ScrH32, rl.Black)
  if level.DiedscrT > 0 {
    level.DiedscrT--
  }
  rl.DrawTexturePro(render.Imgs, player.DiedIMG, player.DiedRec, rl.Vector2Zero(), 0, ranRed())
  rl.DrawTexturePro(render.Imgs, player.DiedIMG, BlurRec(player.DiedRec, 10), rl.Vector2Zero(), 0, rl.Fade(ranRed(), rF32(0.1, 0.4)))
  player.DiedRec.X -= 2
  player.DiedRec.Y -= 2
  player.DiedRec.Width += 4
  player.DiedRec.Height += 4
  if player.DiedRec.Y <= level.LevRecInner.Y || rl.IsKeyPressed(rl.KeySpace) || rl.IsGamepadButtonPressed(0, 7) || rl.IsGamepadButtonPressed(0, 12) && level.DiedscrT == 0 {
    player.Died = false
    timing.BestTime = false
    timing.TimesOn = true
    timing.BestTimesT = core.Fps
  }
  txt := "you"
  txtlen := rl.MeasureText(txt, txU8)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2-3, int32(core.Cnt.Y)-txU8+3, txU8, rl.Black)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2, int32(core.Cnt.Y)-txU8, txU8, rl.White)
  txt = "died"
  txtlen = rl.MeasureText(txt, txU8)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2-3, int32(core.Cnt.Y)+txU+3, txU8, rl.Black)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2, int32(core.Cnt.Y)+txU, txU8, rl.White)
  txt = "new best time"
  txtlen = rl.MeasureText(txt, txU4)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2-3, core.ScrH32-txU5+3, txU4, rl.Black)
  rl.DrawText(txt, int32(core.Cnt.X)-txtlen/2, core.ScrH32-txU5, txU4, rl.White)
}
...
func drawCredits(core *CoreState, level *LevelState, ui *UIState) { //MARK:DRAW CREDITS
	rl.DrawRectangle(0, 0, core.ScrW32, core.ScrH32, rl.Black)
	txt := "credits"
	txtlen := rl.MeasureText(txt, txU5)
	txtx := int32(core.Cnt.X) - txtlen/2
...
func drawExit(core *CoreState, level *LevelState, ui *UIState, timing TimingState, render RenderState, audio AudioState) { //MARK:DRAW EXIT
	txt := "exit"
	txtlen := rl.MeasureText(txt, txU5)
	txtx := int32(core.Cnt.X) - txtlen/2
	txty := int32(core.Cnt.Y) - txU8
	rl.DrawText(txt, txtx, txty, txU5, rl.White)
	txty += txU6
	...
	if rl.IsKeyPressed(rl.KeyA) || rl.IsKeyPressed(rl.KeyLeft) || rl.GetGamepadAxisMovement(0, 0) < 0 && rl.GetGamepadAxisMovement(0, 0) > -0.3 || rl.IsGamepadButtonDown(0, 4) {
		if ui.OptionT == 0 {
			ui.OptionT = core.Fps / 5
			level.ExitLR = !level.ExitLR
		}
	} else if rl.IsKeyPressed(rl.KeyD) || rl.IsKeyPressed(rl.KeyRight) || rl.GetGamepadAxisMovement(0, 0) > 0 && rl.GetGamepadAxisMovement(0, 0) < 0.3 || rl.IsGamepadButtonDown(0, 2) {
		if ui.OptionT == 0 {
			ui.OptionT = core.Fps / 5
...
func drawOptions() { //MARK:DRAW OPTIONS
  ...
  rl.DrawRectangle(0, 0, gs.Core.ScrW32, gs.Core.ScrH32, rl.Black)
  if gs.UI.CreditsOn {
    drawCredits(&gs.Core, &gs.Level, &gs.UI)
  } else if gs.Timing.TimesOn {
    drawTimes()
  } else if gs.UI.HelpOn {
    drawHelp(&gs.Core, &gs.Level, &gs.UI)
  } else if gs.Level.Exiton {
    drawExit(&gs.Core, &gs.Level, &gs.UI, gs.Timing, gs.Render, gs.Audio)
    ...
    gs.UI.HpBarsOn = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.UI.HpBarsOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "scan lines"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.UI.ScanLinesOn = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.UI.ScanLinesOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "pixel artifacts"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.UI.ArtifactsOn = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.UI.ArtifactsOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "bloom 'fuzzy'"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Render.ShaderOn = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.Render.ShaderOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "player atk range"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Player.PlatkrecOn = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.Player.PlatkrecOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "gs.UI.Invincible"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.UI.Invincible = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.UI.Invincible)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "use controller"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Input.UseController = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.Input.UseController)
    ...
    txt = "music"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Audio.MusicOn = onoff(&gs.Core,onoffx, txty, float32(gs.UI.TxtSize), gs.Audio.MusicOn)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2

    txt = "music track"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Audio.BgMusicNum = int(upDownSwitch(&gs.UI,&gs.Audio,onoffx, txty, float32(gs.UI.TxtSize), float32(gs.Audio.BgMusicNum), 2))
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2
    ...
    txt = "volume - 0 is off"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Audio.Volume = upDownSwitch(&gs.UI, &gs.Audio,onoffx, txty, float32(gs.UI.TxtSize), gs.Audio.Volume, 1)
    txty += gs.UI.TxtSize + gs.UI.TxtSize/2
    ...
    txt = "hardcore"
    rl.DrawText(txt, txtx, txty, gs.UI.TxtSize, rl.White)
    gs.Level.Hardcore = onoff(&gs.Core, onoffx, txty, float32(gs.UI.TxtSize), gs.Level.Hardcore)
...
func drawInven(render RenderState, player PlayerState, lvl LevelState, ori rl.Vector2) { //MARK:DRAW INVENTORY

  x := lvl.LevX - bsU2
  y := lvl.LevY + bsU/2
  siz := bsU + bsU/2

  for a := 0; a < len(player.Inven); a++ {
    rec := rl.NewRectangle(x, y, siz, siz)
    rl.DrawTexturePro(render.Imgs, player.Inven[a].img, rec, ori, 0, player.Inven[a].color)

    rl.DrawTexturePro(render.Imgs, player.Inven[a].img, BlurRec(rec, 1), ori, 0, rl.Fade(player.Inven[a].color, rF32(0.1, 0.2)))

    if player.Inven[a].numof > 1 {
      txtx := int32(x+siz) - txU/2
      txty := int32(y+siz) - txU/2
      rl.DrawText(fmt.Sprint(player.Inven[a].numof), txtx-2, txty-2, txU, rl.Black)
      rl.DrawText(fmt.Sprint(player.Inven[a].numof), txtx, txty, txU, rl.White)
    }
...
func drawInvenDetail(
  player PlayerState,
  lvl LevelState,
  core CoreState,
  render RenderState,
) { //MARK:DRAW INVENTORY DETAIL

  if len(player.Inven) > 0 {

    x := lvl.LevX
    y := lvl.LevY + bsU/4
    siz := bsU

    txtcntr := int32(core.Cnt.X)
    txty := int32(y) + txU/4

    for a := 0; a < len(player.Inven); a++ {

      txt := player.Inven[a].name + " - " + player.Inven[a].desc
      ...
      rl.DrawTexturePro(render.Imgs, player.Inven[a].img, rl.NewRectangle(x, y, siz, siz), core.Ori, 0, player.Inven[a].color)

      if player.Inven[a].numof > 1 {
        txtx2 := int32(x+siz) - txU/2
        txty2 := int32(y+siz) - txU/2
        rl.DrawText(fmt.Sprint(player.Inven[a].numof), txtx2-2, txty2-2, txU, rl.Black)
        rl.DrawText(fmt.Sprint(player.Inven[a].numof), txtx2, txty2, txU, rl.White)
      }
...
func drawChainLight(fx *FXState, level *LevelState, core *CoreState, player *PlayerState, audio *AudioState) { //MARK:DRAW CHAIN LIGHTNING
  for a := 1; a < len(fx.ChainV2); a++ {
    rl.DrawLineEx(fx.ChainV2[a], fx.ChainV2[a-1], rF32(2, 12), rl.Fade(ranCyan(), rF32(0.2, 0.5)))
  }
  fx.ChainV2 = nil
  for a := 0; a < len(level.Level[level.RoomNum].enemies); a++ {
    if level.Level[level.RoomNum].enemies[a].cnt.X > level.LevRecInner.X && level.Level[level.RoomNum].enemies[a].cnt.Y > level.LevRecInner.Y {
      fx.ChainV2 = append(fx.ChainV2, level.Level[level.RoomNum].enemies[a].cnt)
    }
  }
  fx.ChainLightTimer--
  if fx.ChainLightTimer <= 0 {
    for a := 0; a < len(level.Level[level.RoomNum].enemies); a++ {
      level.Level[level.RoomNum].enemies[a].hppause = core.Fps / 2
      level.Level[level.RoomNum].enemies[a].hp -= 1
      if level.Level[level.RoomNum].enemies[a].hp <= 0 {
        cntr := level.Level[level.RoomNum].enemies[a].cnt
        addKill(player, *level, a)
        rl.PlaySound(audio.Sfx[7])
        level.Level[level.RoomNum].enemies[a].off = true
        makeFX(2, cntr)
      } else {
        playEnemyHitAudio(*audio)
      }
    }
    fx.ChainLightOn = false
  }
}
...
func drawnocam(gs *GameState) { //MARK:DRAW NO CAM
...
func drawnoRender(player PlayerState, lvl LevelState, core CoreState, render RenderState) { //MARK:DRAW NO RENDER
  if player.InvenOn {
    rl.BeginMode2D(gs.Render.Cam2)

    drawInvenDetail(player, lvl, core, render)
...
func drawDebug(gs GameState) { //MARK:DRAW DEBUG
...
func drawBlokDrec(render *RenderState, core *CoreState, blok xblok, shadow, blur bool, blurDist float32) { //MARK:DRAW BLOK DREC
  if shadow {
    shadowRec := blok.drec
    shadowRec.X -= 5
    shadowRec.Y += 5
    rl.DrawTexturePro(render.Imgs, blok.img, shadowRec, rl.NewVector2(blok.drec.Width/2, blok.drec.Height/2), blok.ro, rl.Fade(rl.Black, 0.8))
  }
  rl.DrawTexturePro(render.Imgs, blok.img, blok.drec, rl.NewVector2(blok.drec.Width/2, blok.drec.Height/2), blok.ro, rl.Fade(blok.color, blok.fade))
  if blur {
    blurRec := blok.drec
    blurRec.X -= rF32(-blurDist, blurDist)
    blurRec.Y -= rF32(-blurDist, blurDist)
    rl.DrawTexturePro(render.Imgs, blok.img, blurRec, rl.NewVector2(blok.drec.Width/2, blok.drec.Height/2), blok.ro, rl.Fade(blok.color, rF32(0.05, 0.2)))
  }
  if core.Debug {
...
func drawBlok(render *RenderState, core *CoreState, blok xblok, shadow, blur bool, blurDist float32) { //MARK:DRAW BLOK
	if shadow {
		shadowRec := blok.rec
		shadowRec.X -= 5
		shadowRec.Y += 5
		rl.DrawTexturePro(render.Imgs, blok.img, shadowRec, rl.NewVector2(0, 0), blok.ro, rl.Fade(rl.Black, 0.8))
	}
	//CANDLE LIGHT
	if blok.name == "candle" {
	...
	//DRAW BLOK IMG
	rl.DrawTexturePro(render.Imgs, blok.img, blok.rec, rl.NewVector2(0, 0), blok.ro, rl.Fade(blok.color, blok.fade))
	if blur {
		if blok.name == "skull" || blok.name == "candle" { //HALF BLUR
			blurRec := blok.rec
			blurRec.X -= rF32(-blurDist/2, blurDist/2)
			blurRec.Y -= rF32(-blurDist/2, blurDist/2)
			rl.DrawTexturePro(render.Imgs, blok.img, blurRec, rl.NewVector2(0, 0), blok.ro, rl.Fade(blok.color, rF32(0.05, 0.2)))
		} else { //FULL BLUR
			blurRec := blok.rec
			blurRec.X -= rF32(-blurDist, blurDist)
			blurRec.Y -= rF32(-blurDist, blurDist)
    ...
    if gs.Enemies.EnProj[a].onoff {
      ...
      if gs.Enemies.EnProj[a].name == "boss1" {
        if rl.CheckCollisionRecs(gs.Enemies.EnProj[a].crec, gs.Player.Pl.crec) {
          hitPL(&gs.Core, &gs.Level, &gs.Render, &gs.Audio, &gs.Player, &gs.Enemies, &gs.UI, a, 1)
        }
      } else {
        ...
        if rl.CheckCollisionRecs(gs.Player.PlProj[a].rec, gs.Level.Bosses[gs.Level.Bossnum].crec) && gs.Level.Bosses[gs.Level.Bossnum].hppause == 0 {
          ...
          if gs.Level.Bosses[gs.Level.Bossnum].hp <= 0 {
            ...
            if !gs.Level.Endgame {
              ...
              addTime(&gs.Timing, gs.Level.MinsEND, gs.Level.SecsEND)
              gs.Level.EndgopherRec = rl.NewRectangle(gs.Core.Cnt.X-bsU4, gs.Level.LevRec.Y+gs.Level.LevRec.Height, bsU8, bsU8)
...
func drawUpEtc() { //MARK:DRAW UP ETC
  ...
  for a := 0; a < len(gs.Level.Level[gs.Level.RoomNum].etc); a++ {
    ...
    if gs.Level.Level[gs.Level.RoomNum].etc[a].onoff {
      ...
      if gs.Level.Level[gs.Level.RoomNum].etc[a].name == "turret" {
        drawBlokDrec(&gs.Render, &gs.Core, gs.Level.Level[gs.Level.RoomNum].etc[a], true, false, 0)

        gs.Level.Level[gs.Level.RoomNum].etc[a].ro++
        if gs.Level.Level[gs.Level.RoomNum].etc[a].timer <= 0 {
          gs.Level.Level[gs.Level.RoomNum].etc[a].timer = rI32(1, 3) * gs.Core.Fps
          makeProjectileEnemy(&gs.Enemies, &gs.Render,1, gs.Level.Level[gs.Level.RoomNum].etc[a].cnt)
        }
      } else if gs.Level.Level[gs.Level.RoomNum].etc[a].name == "spring" {

        drawBlokDrec(&gs.Render, &gs.Core, gs.Level.Level[gs.Level.RoomNum].etc[a], false, true, 4)
        ...
        if rl.CheckCollisionRecs(gs.Player.Pl.crec, gs.Level.Level[gs.Level.RoomNum].etc[a].rec) && gs.Level.Level[gs.Level.RoomNum].etc[a].timer == 0 {
          gs.Level.Level[gs.Level.RoomNum].etc[a].timer = gs.Core.Fps * 3
          makeGasCloud(&gs.Render, &gs.Level, gs.Level.Level[gs.Level.RoomNum].etc[a].cnt)
        ...
        drawBlok(&gs.Render, &gs.Core,  gs.Level.Level[gs.Level.RoomNum].etc[a], false, true, 7)
        ...
        drawBlok(&gs.Render, &gs.Core,gs.Level.Level[gs.Level.RoomNum].etc[a], true, true, 4)
        if gs.Core.Frames%2 == 0 {
        ...
        drawBlok(&gs.Render, &gs.Core,gs.Level.Level[gs.Level.RoomNum].etc[a], true, false, 4)
      } else if gs.Level.Level[gs.Level.RoomNum].etc[a].name == "flamefiretrail" {
        drawBlok(&gs.Render, &gs.Core,gs.Level.Level[gs.Level.RoomNum].etc[a], false, true, 4)
        if gs.Core.Frames%3 == 0 {
        ...
        if gs.Level.Level[gs.Level.RoomNum].etc[a].name == "chest" && gs.Level.Level[gs.Level.RoomNum].etc[a].img.X < 493 {
          ...
          if rl.CheckCollisionRecs(gs.Player.Pl.arec, gs.Level.Level[gs.Level.RoomNum].etc[a].crec) && gs.Player.Mods.key && !gs.Level.Level[gs.Level.RoomNum].etc[a].onoffswitch {
            ...
            delInven(&gs.Player,"key")
            makeChestItem(&gs.Render, &gs.Level, a)
            ...
            txtHere(&gs.UI, "locked", gs.Level.Level[gs.Level.RoomNum].etc[a].rec)
      ...
      if rl.CheckCollisionRecs(gs.Player.Pl.crec, gs.Level.Level[gs.Level.RoomNum].etc[a].rec) {
        ...
        case "gem":
          if gs.Level.Level[gs.Level.RoomNum].etc[a].onoff {
            ...
            gs.Level.Level[gs.Level.RoomNum].etc[a].onoff = false
            txtAddCoin(&gs.UI, &gs.Player.Pl.coins, gs.Audio, gs.Level, "jackpot")
          }
        ...
        case "switch": //MARK: SWITCHES ON/OFF
          if gs.Level.Level[gs.Level.RoomNum].etc[a].timer == 0 {
            ...
            case 1:
              makeSwitchArrows(&gs.Level, &gs.Enemies, &gs.Render)
            case 2:
              gs.Level.Night = !gs.Level.Night
            case 3:
              gs.Level.Flipcam = !gs.Level.Flipcam
              cams(&gs.Render.Cam2, gs.Core, gs.UI, gs.Level)
            case 4:
            ...
            case 6:
              if gs.Level.Level[gs.Level.RoomNum].etc[a].numCoins > 0 {
                ...
                txtAddCoin(&gs.UI, &gs.Player.Pl.coins, gs.Audio, gs.Level, "+1 coin")
              }
  ...
  if gs.Level.Exited {
    gs.Core.Pause = true
    rl.DrawRectangle(0, 0, gs.Core.ScrW32, gs.Core.ScrH32, rl.Black)
    makenewlevel(&gs.Level, &gs.Player, &gs.FX, gs.Companions, &gs.Shop, &gs.Enemies, gs.Core, gs.Render)
...
func drawUpBoss() { //MARK: DRAW UP BOSS
  ...
  if gs.Level.Bosses[gs.Level.Bossnum].timer <= 0 {
    ...
    case 1:
      makeProjectileEnemy(&gs.Enemies, &gs.Render,7, gs.Level.Bosses[gs.Level.Bossnum].cnt)
    case 2:
      makeProjectileEnemy(&gs.Enemies, &gs.Render,8, gs.Level.Bosses[gs.Level.Bossnum].cnt)
    case 3:
      makeProjectileEnemy(&gs.Enemies, &gs.Render,9, gs.Level.Bosses[gs.Level.Bossnum].cnt)
    }
  }
  if gs.Level.Bosses[gs.Level.Bossnum].hppause > 0 {
...
func checkController(input *InputState, ui *UIState, core *CoreState) { //MARK:CHECK CONTROLLER
  input.IsController = rl.IsGamepadAvailable(0)
  if input.IsController && input.ControllerOn {
    input.UseController = true
  } else if !input.IsController {
    input.UseController = false
    input.ControllerDisconnect = true
  }
  if input.IsController {
    input.ControllerOn = true
    input.ControllerDisconnect = false
    input.ControllerWasOn = true
  }
  if input.ControllerDisconnect && input.ControllerWasOn && !core.Pause {
    input.ControllerOn = false
    input.ControllerWasOn = false
    ui.OptionsOn = true
    ui.OptionNum = 0
    core.Pause = true
  }
...
func checkInnerBloksExits(lvl LevelState, roomN int, rec rl.Rectangle) bool { //MARK:CHECK INNER BLOK EXITS

  canadd := true

  for a := 0; a < len(lvl.Level[roomN].doorExitRecs); a++ {
    if rl.CheckCollisionRecs(rec, lvl.Level[roomN].doorExitRecs[a]) {
      canadd = false
    }
  }

  if canadd {
    for a := 0; a < len(lvl.Level[roomN].innerBloks); a++ {
      if rl.CheckCollisionRecs(rec, lvl.Level[roomN].innerBloks[a].rec) {
        canadd = false
      }
    }
  }
...
func findRecPoswithSpacing(level *LevelState, wid, space float32, numRoom int) (rec rl.Rectangle, found bool) { //MARK: FIND REC POS WITH SPACING
  countbreak := 500
  found = true
  wid2 := wid + (space * 2)
  for {
    rec = rl.NewRectangle(rF32(level.LevRecInner.X, level.LevRecInner.X+level.LevRecInner.Width-wid2), rF32(level.LevRecInner.Y, level.LevRecInner.Y+level.LevRecInner.Width-wid2), wid2, wid2)
    canadd := true
    for a := 0; a < len(level.Level[numRoom].doorExitRecs); a++ {
      if rl.CheckCollisionRecs(rec, level.Level[numRoom].doorExitRecs[a]) {
        canadd = false
      }
    }
    if canadd {
      for a := 0; a < len(level.Level[numRoom].movBloks); a++ {
        if rl.CheckCollisionRecs(rec, level.Level[numRoom].movBloks[a].rec) {
          canadd = false
        }
      }
    }
    if canadd {
      for a := 0; a < len(level.Level[numRoom].innerBloks); a++ {
        if rl.CheckCollisionRecs(rec, level.Level[numRoom].innerBloks[a].rec) {
          canadd = false
        }
      }
    }
    if canadd {
      for a := 0; a < len(level.Level[numRoom].etc); a++ {
        if rl.CheckCollisionRecs(rec, level.Level[numRoom].etc[a].rec) && level.Level[numRoom].etc[a].solid {
          canadd = false
        }
      }
    }
...
func findRecPos(level *LevelState, wid float32, numRoom int) (rec rl.Rectangle, found bool) { //MARK: FIND REC POS
	countbreak := 500
	found = true
	for {
		rec = rl.NewRectangle(rF32(level.LevRecInner.X, level.LevRecInner.X+level.LevRecInner.Width-wid), rF32(level.LevRecInner.Y, level.LevRecInner.Y+level.LevRecInner.Width-wid), wid, wid)
		canadd := true
		for a := 0; a < len(level.Level[numRoom].movBloks); a++ {
			if rl.CheckCollisionRecs(rec, level.Level[numRoom].movBloks[a].rec) {
				canadd = false
...
func findRanRecLoc(level *LevelState, w, h float32, roomNum int) (tl rl.Vector2) { //MARK: FIND RANDOM RECTANGLE LOCATION
  v2 := rl.Vector2{}
  countbreak := 100
  for {
    v2 = rl.NewVector2(level.LevRecInner.X+bsU2, level.LevRecInner.Y+bsU2)
    wid := level.LevRecInner.Width - bsU4
    heig := wid
    wid -= w
    heig -= h
    v2.X += rF32(0, wid)
    ...
    canadd := true
    for a := 0; a < len(level.Level[roomNum].doorExitRecs); a++ {
      if rl.CheckCollisionRecs(checkrec, level.Level[roomNum].doorExitRecs[a]) {
        canadd = false
      }
    }
    countbreak--
      ...
      for a := 0; a < len(gs.Level.Level[gs.Level.RoomNum].etc); a++ {
        if gs.Level.Level[gs.Level.RoomNum].etc[a].name == "footprints" && gs.Level.Level[gs.Level.RoomNum].enemies[num].hppause == 0 && rl.CheckCollisionRecs(gs.Level.Level[gs.Level.RoomNum].etc[a].rec, gs.Level.Level[gs.Level.RoomNum].enemies[num].crec) {
          ...
          if gs.Level.Level[gs.Level.RoomNum].enemies[num].hp <= 0 {
            ...
            addKill(&gs.Player, gs.Level, num)
            rl.PlaySound(gs.Audio.Sfx[7])
...
func uplevel(gs *GameState) { //MARK:MOVE BLOKS
...
func savesettings(ui UIState, render RenderState, player PlayerState, level LevelState, audio AudioState, input InputState) { //MARK:SAVE SETTINGS
  ...
  musicTXT := fmt.Sprint(audio.BgMusicNum)
  settingsTXT = settingsTXT + musicTXT + ","

  voltxt := fmt.Sprintf("%.0f", audio.Volume*10)
...
func addTime(timing *TimingState, minsEND int, secsEND int) { //MARK:ADD TIME
  totaltime := minsEND*60 + secsEND
  checktime := totaltime
  canadd := false
  changenum := 0
  for i := 0; i < len(timing.Times); i++ {
    if checktime < timing.Times[i] {
      canadd = true
      checktime = timing.Times[i]
      changenum = i
    }
  }
  if canadd {
    timing.Times[changenum] = totaltime
    sort.Ints(timing.Times)
    timing.BestTime = true
    saveTimes(*timing)
  }
}

func restartgame(gs *GameState) { //MARK: RESTART GAME
  ...
  makePlayer(&gs.Player, &gs.Render, &gs.Core)
  makelevel(&gs.Level, &gs.Shop, &gs.Enemies, gs.Core, gs.Render)
...
func saveTimes(time TimingState) { //MARK: SAVE TIMES
  ...
  scoresTXT := ""
  for i := 0; i < len(time.Times); i++ {
    if i == len(time.Times)-1 {
      scoresTXT = scoresTXT + fmt.Sprint(time.Times[i])
    } else {
      scoresTXT = scoresTXT + fmt.Sprint(time.Times[i]) + ","
    }
  }
...
func exitGame(timing TimingState, render RenderState, audio AudioState) { //MARK: EXIT GAME

  if gs.UI.OptionsChange {
    savesettings(gs.UI, gs.Render, gs.Player, gs.Level, gs.Audio, gs.Input)
  }
  saveTimes(timing)
  ...
  if len(gs.Player.Inven) > 0 {
    ...
    for a := 0; a < len(gs.Player.Inven); a++ {
      if gs.Shop.ShopItems[num].name == gs.Player.Inven[a].name {
        ...
        case "fireworks":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level,"fireworks")
          sold = true
        case "peace":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level,"peace")
          sold = true
        case "anchor":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "anchor")
          sold = true
        case "recharge":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level,"recharge")
          sold = true
        case "orbital":
          if gs.Player.Mods.orbitalN < gs.Player.Max.orbital {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level,"orbital")
            sold = true
          }
        case "coffee":
          if gs.Player.Mods.coffeeN < gs.Player.Max.coffee {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level,"coffee")
            sold = true
          }
        case "invisible":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level,"invisible")
          sold = true
        case "health potion":
          if gs.Player.Mods.hppotionN < gs.Player.Max.hppotion {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "health potion")
            sold = true
          }
        case "firetrail":
          if gs.Player.Mods.firetrailN < gs.Player.Max.firetrail {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "firetrail")
            sold = true
          }
        case "map":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "map")
          sold = true
        case "apple":
        ...
        case "fireball":
          if gs.Player.Mods.fireballN < gs.Player.Max.fireball {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "fireball")
            sold = true
          }
        case "throwing axe":
...
func upDownSwitch(ui *UIState, audio *AudioState, x32, y32 int32, siz, value float32, numType int) float32 { //MARK:UP DOWN SWITCH
	x := float32(x32)
	y := float32(y32)
	...
	case 2:
		...
		txtlen := rl.MeasureText(txt, ui.TxtSize)
		rl.DrawText(txt, (rec.ToInt32().X+rec.ToInt32().Width/2)-txtlen/2, rec.ToInt32().Y+1, ui.TxtSize, rl.White)
	case 1:
...
func collectInven(blokNum int) { //MARK:COLLECT INVENTORY
  ...
  if len(gs.Player.Inven) > 0 {
    ...
    for a := 0; a < len(gs.Player.Inven); a++ {
      if gs.Level.Level[gs.Level.RoomNum].etc[blokNum].name == gs.Player.Inven[a].name {
        ...
        case "mr carrot":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "mr carrot")
          sold = true
        case "fireworks":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "fireworks")
        ...
        case "air strike":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "air strike")
          sold = true
        case "mr alien":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "mr alien")
          sold = true
        case "peace":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "peace")
        ...
        case "birthday cake":
          if gs.Player.Mods.cakeN < gs.Player.Max.cake {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "birthday cake")
            sold = true
          }
        case "fish":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "fish")
          sold = true
        case "cherry":
          if gs.Player.Mods.cherryN < gs.Player.Max.cherry {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "cherry")
            sold = true
          }
        case "socks":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "socks")
          sold = true
        case "umbrella":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "umbrella")
          sold = true
        case "anchor":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "anchor")
        ...
        case "health ring":
          if gs.Player.Mods.hpringN < gs.Player.Max.hpring {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "health ring")
            sold = true
          }
        case "chain lightning":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "chain lightning")
          sold = true
        case "orbital":
          if gs.Player.Mods.orbitalN < gs.Player.Max.orbital {
            gs.Player.Inven[a].numof++
        ...
        case "attack damage":
          if gs.Player.Mods.atkdmgN < gs.Player.Max.atkdmg {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "attack damage")
            sold = true
          }
        case "attack range":
          if gs.Player.Mods.atkrangeN < gs.Player.Max.atkrange {
            ...
            gs.Player.Pl.coins++
            txtSold(&gs.UI, gs.Level, "attack range")
            sold = true
          }
        case "coffee":
          if gs.Player.Mods.coffeeN < gs.Player.Max.coffee {
        ...
        case "wallet":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "wallet")
          sold = true
        case "medi kit":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "medi kit")
          sold = true
        case "mr planty":
          gs.Player.Pl.coins++
          txtSold(&gs.UI, gs.Level, "mr planty")
          sold = true
        case "apple":
          if gs.Player.Mods.appleN < gs.Player.Max.apple {
            gs.Player.Inven[a].numof++
  ...
  if !sold {
    ...
    case "fish":
      ...
      makeFish(&gs.FX, gs.Core, gs.Render)
    case "cherry":
      if gs.Player.Mods.cherryN < gs.Player.Max.cherry {
...
func txtHere(ui *UIState, txt string, rec rl.Rectangle) { //MARK:TEXT HERE
  ...
  ui.GameTxt = append(ui.GameTxt, ztxt)
}

func onoff(core *CoreState, x32, y32 int32, siz float32, name bool) bool { //MARK: ON/OFF
  x := float32(x32)
  y := float32(y32)
  ...
  rl.DrawRectangleLinesEx(rec, 1, rl.White)
  if rl.CheckCollisionPointRec(core.Mousev2cam, rec) {
...
func clearInven(player *PlayerState, name string) { //MARK:CLEAR INVEN

  num := 0
  clear := false

  for a := 0; a < len(player.Inven); a++ {
    if player.Inven[a].name == name {
      num = a
      clear = true
    }
  }
  if clear {
    player.Inven = remBlok(player.Inven, num)
  }

}

func delInven(player *PlayerState, name string) { //MARK:DEL INVEN
  should_clear := false
  for a := 0; a < len(player.Inven); a++ {
    if name == player.Inven[a].name {
      player.Inven[a].numof--
      if player.Inven[a].numof == 0 {
        should_clear = true
      }
    }
  }
  ...
  if should_clear {
    clearInven(player, name)
  }

}

func escapePlayer(player *PlayerState, enemies *EnemyState, fx *FXState, core *CoreState, level *LevelState, render *RenderState) { //MARK:ESCAPE PLAYER
	if player.Pl.cnt.Y > level.LevRecInner.Y && !player.Escaped {
		player.Pl.cnt.Y -= bsU / 4
		if player.Pl.cnt.X > core.Cnt.X {
			player.Pl.cnt.X -= bsU / 4
		} else if player.Pl.cnt.X < core.Cnt.X {
			player.Pl.cnt.X += bsU / 4
		}
		upPlayerRec(&player.Pl)
		if player.Pl.cnt.Y <= level.LevRecInner.Y {
			player.Escaped = true
			upRoomChange(player, enemies, fx, core, level, render)
			player.Pl.hp = 2
		}
	}
	if player.Escaped {
		if !player.EscapeRoomFound {
			choose := 0
			for {
				choose = rInt(0, len(level.Level))
				if choose != level.RoomNum {
					break
				}
			}
			level.RoomNum = choose
			player.EscapeRoomFound = true
		}
		if player.Pl.cnt.Y < level.LevRecInner.Y+bsU3 {
			player.Pl.cnt.Y += bsU / 4
		}
		if player.Pl.cnt.X > core.Cnt.X {
			player.Pl.cnt.X -= bsU / 4
		...
		upPlayerRec(&player.Pl)
		if player.Pl.cnt.Y >= level.LevRecInner.Y+bsU3 {
			player.Pl.escape = false
		}
	}
}

func cleanLevel(level *LevelState, core CoreState) { //MARK:CLEAN LEVEL
	//EMPTY ETC BLOKS
	for a := 0; a < len(level.Level); a++ {
		var blokstoClear []int
		for b := 0; b < len(level.Level[a].etc); b++ {
			if level.Level[a].etc[b].name == "" {
				blokstoClear = append(blokstoClear, b)
			}
		}
		if len(blokstoClear) > 0 {
			for b := 0; b < len(blokstoClear); b++ {
				level.Level[a].etc = remBlok(level.Level[a].etc, blokstoClear[b])
			}
		}
	}
	...
	num := 0
	clear := false
	checkrec := rl.NewRectangle(core.Cnt.X-bsU4, core.Cnt.Y-bsU4, bsU8, bsU8)
	if len(level.Level[0].innerBloks) > 0 {
		for a := 0; a < len(level.Level[0].innerBloks); a++ {
			if rl.CheckCollisionRecs(checkrec, level.Level[0].innerBloks[a].rec) {
				num = a
				clear = true
			}
		}
	...
	if clear {
		level.Level[0].innerBloks = remBlok(level.Level[0].innerBloks, num)
	}
	//CLEAR ETC CENTER
	for i := 0; i < len(level.Level[0].etc); i++ {
		checkrec := rl.NewRectangle(core.Cnt.X-bsU2, core.Cnt.Y-bsU2, bsU4, bsU4)
		if rl.CheckCollisionRecs(checkrec, level.Level[0].etc[i].rec) {
			level.Level[0].etc[i].onoff = false
		}
	}
	//CLEAR MOVE BLOCKS CENTER
	for i := 0; i < len(level.Level[0].movBloks); i++ {
		checkrec := rl.NewRectangle(core.Cnt.X-bsU2, core.Cnt.Y-bsU2, bsU4, bsU4)
		if rl.CheckCollisionRecs(checkrec, level.Level[0].movBloks[i].rec) {
			level.Level[0].movBloks[i].rec.X -= bsU4
		}
	}
}

func hitPL(core *CoreState, level *LevelState, render *RenderState, audio *AudioState, player *PlayerState, enemies *EnemyState, ui *UIState, numEnProj, numType int) { //MARK:HIT PLAYER
	if player.Pl.hppause == 0 && player.Pl.peaceT == 0 && player.StartdmgT == 0 {
		player.Pl.hppause = core.Fps * 2
		player.HpHitY = 0
		player.HpHitF = 1
		if player.Pl.armor > 0 {
			player.Pl.armor--
			player.Pl.armorHit = true
		} else {
			switch numType {
			...
			case 1: //ENEMY PROJECTILE
				if !ui.Invincible {
					player.Pl.hp -= enemies.EnProj[numEnProj].dmg
				}
			}
			zblok := xblok{}
			zblok.rec = player.Pl.rec
			zblok.cnt = player.Pl.cnt
			zblok.img = render.Splats[rInt(0, len(render.Splats))]
			zblok.color = ranRed()
			zblok.fade = 0.5
			zblok.onoff = true
			zblok.name = "playerblood"
			level.Level[level.RoomNum].etc = append(level.Level[level.RoomNum].etc, zblok)
			if player.Pl.hp <= 0 && !ui.Invincible {
				player.Pl.hp = 0
				if player.Mods.medikit {
					player.Pl.hp = player.Pl.hpmax
					player.Pl.revived = true
					player.Pl.hppause = core.Fps * 3
					player.ReviveY = 0
					player.ReviveF = 1
					player.Mods.medikit = false
					clearInven(player, "medi kit")
				} else {
					level.DiedscrT = core.Fps * 3
					core.Pause = true
					player.Died = true
					player.DiedRec = rl.NewRectangle(core.Cnt.X-bsU, core.Cnt.Y-bsU, bsU2, bsU2)
					player.DiedIMG = render.Splats[rInt(0, len(render.Splats))]
				}
			}
			rl.PlaySound(audio.Sfx[14])
		}
	}
}

func addKill(player *PlayerState, lvl LevelState, num int) { //MARK:ADD KILL
  switch lvl.Level[lvl.RoomNum].enemies[num].name {
  case "rabbit1":
    player.Kills.bunnies++
  case "bat":
    player.Kills.bats++
  case "mushroom":
    player.Kills.mushrooms++
  case "ghost":
    player.Kills.ghosts++
  case "spikehog":
    player.Kills.spikehogs++
  case "rock":
    player.Kills.rocks++
  case "slime":
    player.Kills.slimes++
...
func txtSold(ui *UIState, level LevelState, name string) { //MARK:TEXT SOLD
  rl.PlaySound(gs.Audio.Sfx[18])
  ztxt := xtxt{}
  ztxt.onoff = true
  ztxt.col = rl.White
  ztxt.fade = 1
  ztxt.y = int32(level.LevY+level.LevW) - bsU5i32
  ztxt.x = int32(level.LevX+level.LevW) + bsUi32

  ztxt.txt = name + " max"
  ztxt.txt2 = "extra sold"

  ui.TxtSoldList = append(ui.TxtSoldList, ztxt)

}

func txtCompanion(ui *UIState, level LevelState) { //MARK:TEXT COMPANION
  ztxt := xtxt{}
  ztxt.onoff = true
  ztxt.col = rl.White
  ztxt.fade = 1
...
func txtAddCoin(
  ...
  level LevelState,
  txt string,
) { //MARK:TEXT ADD 1 COIN
  rl.PlaySound(audio.Sfx[18])
  ztxt := xtxt{}
  ztxt.onoff = true
  ztxt.col = rl.White
  ztxt.fade = 1
...
func up(gs *GameState) { //MARK:UP

  if !gs.Core.Pause {
    uplevel(gs)
    upplayer(gs)
    timers(&gs.Shop, &gs.Level, &gs.Core)
  }

  checkController(&gs.Input, &gs.UI, &gs.Core)
...
func upPlayerMods(fx *FXState, player *PlayerState, mario *MarioState, level *LevelState, core CoreState, render RenderState, audio AudioState) { //MARK:UP PLAYER MODS

  //AIR STRIKE
  if player.Mods.airstrike {
    fx.AirstrikeT--
    if fx.AirstrikeT <= 0 {
      fx.AirstrikeT = core.Fps * rI32(3, 8)
      makeAirstrike(fx, level, core)
    }

  }

  //FLOOD
  if player.Mods.flood {
    if mario.MarioOn {
      fx.FloodRec.Y = core.ScrHF32 + bsU
    } else if level.Levelnum == 6 {
      mario.MarioOn = false
    } else {
      levrecV2WorldtoScreen := rl.GetWorldToScreen2D(rl.NewVector2(level.LevRecInner.X, level.LevRecInner.Y), render.Cam2)
      if fx.FloodRec.Y > levrecV2WorldtoScreen.Y+bsU12 {
        fx.FloodRec.Y -= 4
      } else {
        if fx.FishLR {
          fx.FishV2.X -= 5
          if fx.FishV2.X < -fx.FishSiz {
            fx.FishLR = false
            fx.Fish1 = render.FishR.recTL
            fx.FishV2.Y = rF32(core.ScrHF32/3, core.ScrHF32)
          }
        } else {
          fx.FishV2.X += 7
          if fx.FishV2.X > core.ScrWF32 {
            fx.FishLR = true
            fx.Fish1 = render.FishL.recTL
            fx.FishV2.Y = rF32(core.ScrHF32/3, core.ScrHF32)
          }
        }
        if fx.Fish2LR {
          fx.Fish2V2.X += 4
          if fx.Fish2V2.X > core.ScrWF32 {
            fx.Fish2LR = false
            fx.Fish2 = render.FishL.recTL
            fx.Fish2V2.Y = rF32(core.ScrHF32/3, core.ScrHF32)
          }
        } else {
          fx.Fish2V2.X -= 7
          if fx.Fish2V2.X <= 0 {
            fx.Fish2LR = true
            fx.Fish2 = render.FishR.recTL
            fx.Fish2V2.Y = rF32(core.ScrHF32/3, core.ScrHF32)
          }
        }
      }
  ...
  //SOCKS
  if player.Mods.socks {
    sockstimer := 30
    if core.Frames%sockstimer == 0 {
      zblok := makeBlokGenNoRecNoCntr()
      zblok.fade = 0.7
      zblok.color = ranGreen()
      zblok.cnt = player.Pl.cnt
      ...
      zblok.img = render.Etc[52]
      zblok.name = "footprints"
      switch player.Pl.direc {
      case 2:
        zblok.ro = 90
      case 3:
        zblok.ro = 180
  ...
  //FIRETRAIL
  if player.Mods.firetrail {

    flametimer := 60
    if player.Mods.firetrailN == 2 {
      flametimer = 30
    } else if player.Mods.firetrailN == 3 {
      flametimer = 15
    }

    if core.Frames%flametimer == 0 {
      zblok := makeBlokGenNoRecNoCntr()
      zblok.color = ranOrange()
      zblok.cnt = player.Pl.cnt
      siz := bsU2
      zblok.rec = rl.NewRectangle(zblok.cnt.X-siz/2, zblok.cnt.Y, siz, siz)
      zblok.img = render.Firetrailanim.recTL
  ...
  //HP POTION
  if player.Mods.hppotion && player.Pl.hppotionT == 0 {
    if player.Pl.hp <= 2 {
      player.Pl.hppotionT = core.Fps
      player.Pl.hp = player.Pl.hpmax
      player.Mods.hppotionN--
      player.Pl.hppause = core.Fps * 3
      if player.Mods.hppotionN <= 0 {
        player.Mods.hppotion = false
        clearInven(player, "health potion")
      }
    }
  }

  //ESCAPE VINE
  if player.Pl.hp == 1 && player.Mods.vine && level.Levelnum != 6 {
    rl.PlaySound(audio.Sfx[27])
    player.Mods.vine = false
    clearInven(player, "vine")
    player.Escaped = false
    player.EscapeRoomFound = false
    player.Pl.escape = true
    player.Pl.hppause = core.Fps * 5
  }

  //SANTA
  if player.Mods.santa {
    player.Mods.santaT--
    if player.Mods.santaT <= 0 {
      if player.Mods.snowon {
        player.Mods.snowon = false
      } else {
        makeSnow(core, level, render, fx)
        player.Mods.snowon = true
      }
      player.Mods.santaT = rI32(7, 21) * core.Fps
    }
  }
  //AXE
  if player.Mods.axe {
    player.Mods.axeT2--
    if player.Mods.axeT2 <= 0 {
      player.Mods.axeT2 = player.Mods.axeT
      makeProjectile("axe")
    }
  }
...
func upRoomChange(player *PlayerState, enemies *EnemyState, fx *FXState, core *CoreState, level *LevelState, render *RenderState) { //MARK:UP ROOM CHANGE
  player.PlProj = nil
  enemies.EnProj = nil
  fx.Fx = nil
  //FLOOD
  if player.Mods.flood {
    player.Pl.underWater = false
    fx.FloodRec = rl.NewRectangle(0, core.ScrHF32+bsU, core.ScrWF32, core.ScrHF32)
    makeFish(fx, *core, *render)
  }
  //ARMOR RECHARGE
  if player.Mods.recharge && player.Mods.armorN > 0 {
    player.Pl.rechargeN++
    if player.Pl.rechargeN == 2 {
      player.Pl.rechargeN = 0
      if player.Pl.armor < player.Pl.armorMax {
        player.Pl.armor++
      }
    }
  }
  //PEACE PAUSE
  if player.Mods.peace {
    player.Pl.peaceT = core.Fps * 2
  }
  //ANCHOR PAUSE
  if player.Mods.anchor {
    level.AnchorT = core.Fps * 2
  }
}

func upplayer(gs *GameState) { //MARK:UP PLAYER
  ...
  //ESCAPE
  if gs.Player.Pl.escape {
    escapePlayer(&gs.Player, &gs.Enemies, &gs.FX, &gs.Core, &gs.Level, &gs.Render)
  }
  ...
  //UP MODS
  upPlayerMods(&gs.FX, &gs.Player, &gs.Mario, &gs.Level, gs.Core, gs.Render, gs.Audio)
  ...
  if !gs.Level.RoomChanged {
    if !rl.CheckCollisionPointRec(gs.Player.Pl.cnt, gs.Level.LevRec) {
      ...
      upRoomChange(&gs.Player, &gs.Enemies, &gs.FX, &gs.Core, &gs.Level, &gs.Render)
      cntCompanion := gs.Player.Pl.cnt
...
func makeAudio(audio *AudioState) { //MARK:MAKE AUDIO
  audio.BackMusic = append(audio.BackMusic, rl.LoadMusicStream("audio/1.ogg"))  //0
  audio.BackMusic = append(audio.BackMusic, rl.LoadMusicStream("audio/2.ogg"))  //1
  audio.BackMusic = append(audio.BackMusic, rl.LoadMusicStream("audio/16.ogg")) //2
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/3.ogg"))  //0 PLAYER SWORD
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/4.ogg"))  //1 ENEMY HIT1
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/5.ogg"))  //2 ENEMY HIT2
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/6.ogg"))  //3 SPRING
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/7.ogg"))  //4 SPEAR
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/8.ogg"))  //5 OIL BARREL BURN
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/9.ogg"))  //6 POWER UP BLOCK DESTROY
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/10.ogg")) //7 ENEMY DEATH
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/11.ogg")) //8 ITEM PICKUP
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/12.ogg")) //9 TELEPORT
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/13.ogg")) //10 OIL BARREL EXPLODE
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/14.ogg")) //11 LIGHTNING
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/15.ogg")) //12 SWITCH
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/17.ogg")) //13 COUNTDOWN
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/18.ogg")) //14 PLAYER HIT
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/19.ogg")) //15 STAIRS
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/20.ogg")) //16 SHOP DOOR
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/21.ogg")) //17 CHEST OPENING
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/22.ogg")) //18 COIN ADDED
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/23.ogg")) //19 WIN GAME
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/24.ogg")) //20 GAS TRAP
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/25.ogg")) //21 SHOP PURCHASE
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/26.ogg")) //22 NO MONEY SHOP PURCHASE
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/27.ogg")) //23 POISONED
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/28.ogg")) //24 UNDERWATER
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/29.ogg")) //25 LOCKED CHEST
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/30.ogg")) //26 SPEAR
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/31.ogg")) //27 VINE
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/32.ogg")) //28 AIR STRIKE EXPLOSION
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/33.ogg")) //29 BOSS HIT
  audio.Sfx = append(audio.Sfx, rl.LoadSound("audio/34.ogg")) //30 CIRCULAR SAW
  upVolume(*audio)
}

func makeChestItem(render *RenderState, level *LevelState, num int) { //MARK:MAKE CHEST ITEM
  newcnt := level.Level[level.RoomNum].etc[num].cnt
  newcnt.Y -= bsU2
  zblok := makeBlokGeneric(bsU+bsU/2, newcnt)
  choose := rInt(0, len(render.Gems))
  zblok.onoff = true
  zblok.numof = 1
  zblok.name = "gem"
  zblok.color = rl.White
  zblok.img = render.Gems[choose]
  switch choose {
  case 0:
    zblok.numCoins = 5
  case 1:
...
func makeTimes(time *TimingState) { //MARK: MAKE TIMES
  ...
  for i := 0; i < len(txtTimes); i++ {
    ...
    time.Times = append(time.Times, num)
    sort.Ints(time.Times)
  }

}

func makeSettings(ui *UIState, render *RenderState, player *PlayerState, input *InputState, audio *AudioState, level *LevelState) { //MARK: MAKE SETTINGS
  ...
  if txtSettings[1] == "1" {
    ...
    ui.ScanLinesOn = false
  }
  if txtSettings[2] == "1" {
    ui.ArtifactsOn = true
  } else {
    ui.ArtifactsOn = false
  }
  if txtSettings[3] == "1" {
    render.ShaderOn = true
  } else {
    render.ShaderOn = false
  }
  if txtSettings[4] == "1" {
    player.PlatkrecOn = true
  } else {
    player.PlatkrecOn = false
  }
  if txtSettings[5] == "1" {
    ui.Invincible = true
  } else {
  ...
  audio.BgMusicNum = num
  //VOLUME
  num, _ = strconv.Atoi(txtSettings[9])
  audio.Volume = float32(num) / 10
}

func makeshop(shop *ShopState, level *LevelState, render RenderState, cnt rl.Vector2) { //MARK: MAKE SHOP
  ...
  //SHOP IMG
  for {
    level.ShopRoomNum = rInt(0, len(level.Level))

    zblok := xblok{}
    zblok.name = "shop"
    zblok.solid = true
    zblok.onoff = true
    zblok.img = render.Etc[14]
    zblok.cnt = cnt
    zblok.cnt.Y -= bsU8
    ...
    canadd := true
    zblok.rec, canadd = findRecPoswithSpacing(level, siz, bsU4, level.ShopRoomNum)
    ...
    countbreak--
    if canadd || countbreak == 0 {
      level.Level[level.ShopRoomNum].etc = append(level.Level[level.ShopRoomNum].etc, zblok)
      break
    }
  }
  ...
  for num > 0 {
    ...
    case 15: //FIREWORKS
      ...
      zblok.desc = "shoot fireworks when activating powerup block"
      zblok.color = rl.White
      zblok.img = render.Etc[49]
    case 14: //PEACE
      zblok.name = "peace"
      zblok.desc = "take no damage for 2 seconds on entering room"
      zblok.color = rl.White
      zblok.img = render.Etc[46]
    case 13: //ANCHOR
      zblok.name = "anchor"
      zblok.desc = "enemies pause for 2 seconds on entering room"
      zblok.color = rl.White
      zblok.img = render.Etc[39]
    case 12: //RECHARGE
      zblok.name = "recharge"
      zblok.desc = "only works with armor - recharges 1 armor every 2 rooms"
      zblok.color = rl.White
      zblok.img = render.Etc[12]
    case 11: //ORBITAL
      zblok.name = "orbital"
      zblok.desc = "erratic revolving orbs that damage enemies"
      zblok.color = rl.White
      zblok.img = render.Etc[35]
    case 10: //COFFEE
      zblok.name = "coffee"
      zblok.desc = "move faster - collect more = faster movement"
      zblok.color = rl.White
      zblok.img = render.Etc[31]
    case 9: //INVISIBLE
      zblok.name = "invisible"
      zblok.desc = "enemies will not follow you"
      zblok.color = rl.White
      zblok.img = render.Etc[30]
    case 8: //FIRE TRAIL
      zblok.name = "firetrail"
      zblok.desc = "trail of fire - does not effect flying enemies"
      zblok.color = ranOrange()
      zblok.img = render.Etc[29]
    case 7: //FIREBALL
      zblok.name = "fireball"
      zblok.desc = "fires on attack - collect more = more fireballs"
      zblok.color = ranOrange()
      zblok.img = render.FireballPlayer.recTL
    case 6: //MAP
      zblok.name = "map"
      zblok.desc = "reveals location of exit room"
      zblok.color = ranGrey()
      zblok.img = render.Etc[28]
    case 5: //APPLE
      zblok.name = "apple"
      zblok.desc = "prevents poisoning - destroyed on use"
      zblok.color = rl.White
      zblok.img = render.Etc[8]
    case 4: //SKELETON KEY
      zblok.name = "key"
      zblok.desc = "open locked things"
      zblok.color = rl.White
      zblok.img = render.Etc[7]
    case 3: //BOUNCE PROJECTILE
      zblok.name = "bounce"
      zblok.desc = "projectiles bounce > collect more = more bounces"
      zblok.color = rl.Yellow
      zblok.img = render.Etc[5]
    case 2: //THROWING AXE
      zblok.name = "throwing axe"
      zblok.desc = "fires at interval > collect more = faster fire rate"
      zblok.color = rl.SkyBlue
      zblok.img = render.Etc[3]
    case 1: //HP POTION
      zblok.name = "health potion"
      zblok.desc = "automatically used when health < 2"
      zblok.color = rl.Red
      zblok.img = render.Etc[1]
    }

    if len(shop.ShopItems) > 0 {
      canadd := true
      for a := 0; a < len(shop.ShopItems); a++ {
        if shop.ShopItems[a].name == zblok.name {
          canadd = false
        }
      }
      if canadd {
        zblok.shopprice = rInt(2, 8)
        shop.ShopItems = append(shop.ShopItems, zblok)
        num--
      }
    } else {
      zblok.shopprice = rInt(2, 8)
...
func makeProjectileEnemy(enemies *EnemyState, render *RenderState, num int, cnt rl.Vector2) { //MARK:MAKE PROJECTILE ENEMY
	switch num {
	...
	case 1: //TURRET
		...
		zproj.dmg = 1
		zproj.fade = 1
		zproj.img = render.Etc[19]
		zproj.onoff = true
		enemies.EnProj = append(enemies.EnProj, zproj)
	...
	case 8: //BOSS ATK 2
		...
		zproj.dmg = 1
		zproj.fade = 1
		zproj.img = render.Boss2anim.recTL
		zproj.onoff = true
		enemies.EnProj = append(enemies.EnProj, zproj)
		zproj.velx = rF32(-zproj.vel, zproj.vel)
		zproj.vely = rF32(-zproj.vel, zproj.vel)
	...
	case 7: //BOSS ATK 1
		...
		zproj.dmg = 1
		zproj.fade = 1
		zproj.img = render.Boss1anim.recTL
		zproj.onoff = true
		enemies.EnProj = append(enemies.EnProj, zproj)
		zproj.velx = rF32(-zproj.vel, zproj.vel)
		zproj.vely = rF32(-zproj.vel, zproj.vel)
	...
	case 2, 3, 4, 5: //SWITCH ARROWS
		...
		zproj.dmg = 1
		zproj.fade = 1
		zproj.img = render.Etc[24]
		zproj.onoff = true
		enemies.EnProj = append(enemies.EnProj, zproj)
	}
}

func makeendlevel(level *LevelState, core CoreState, render RenderState ) { //MARK: MAKE END LEVEL
  ...
  level.Bossnum = rInt(0, len(level.Bosses))

  countedBorderBlocks := false

  floorT := render.Floortiles[rInt(0, len(render.Floortiles))]
  level.WallT = render.Walltiles[rInt(0, len(render.Walltiles))]

  zroom := xroom{}
  zroom.floorT = floorT
  zroom.wallT = level.WallT

  //BOUNDARY WALLS
  x := level.LevX
  y := level.LevY

  for x < level.LevX+level.LevW {
    if !countedBorderBlocks {
      level.LevBorderBlokNum++
    }
    zblok := xblok{}
    zblok.fade = 1
    ...
    zblok.rec = rl.NewRectangle(x, y, level.BorderWallBlokSiz, level.BorderWallBlokSiz)
    zroom.walls = append(zroom.walls, zblok)
    zblok.rec.Y = level.LevY + level.LevW - level.BorderWallBlokSiz
    switch level.Levelnum {
    case 1:
      zblok.color = ranBlue()
    case 2:
    ...
    zroom.walls = append(zroom.walls, zblok)
    x += level.BorderWallBlokSiz
  }
  if !countedBorderBlocks {
    countedBorderBlocks = true
  }
  x = level.LevX
  y = level.LevY + level.BorderWallBlokSiz
  for y < level.LevY+level.LevW-level.BorderWallBlokSiz {
    zblok := xblok{}
    zblok.fade = 1
    zblok.img = zroom.wallT
    switch level.Levelnum {
    ...
    zblok.rec = rl.NewRectangle(x, y, level.BorderWallBlokSiz, level.BorderWallBlokSiz)
    zroom.walls = append(zroom.walls, zblok)
    zblok.rec.X = level.LevX + level.LevW - level.BorderWallBlokSiz
    switch level.Levelnum {
    case 1:
      zblok.color = ranBlue()
    case 2:
  ...
  for {
    ...
    x += siz
    if x >= level.LevX+level.LevW {
      x = level.LevX
      y += siz
    }
    if y >= level.LevY+level.LevW {
      break
    }
  }

  level.Level = append(level.Level, zroom)

  //SWITCHES
  if flipcoin() {
    zblok := makeBlokGenNoRecNoCntr()
    canadd := true
    zblok.rec, canadd = findRecPoswithSpacing(level, bsU2, bsU/2, 0)
    zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
    zblok.name = "switch"
    zblok.numType = roll6()
    if zblok.numType == 6 {
    ...
    if zblok.onoffswitch {
      zblok.img = render.Etc[21]
    } else {
      zblok.img = render.Etc[22]
    }
    if canadd {
      level.Level[0].etc = append(level.Level[0].etc, zblok)
    }
  }
  if flipcoin() {
    zblok := makeBlokGenNoRecNoCntr()
  ...
  for {
    ...
    canadd := true
    zblok.rec, canadd = findRecPos(level, siz, 0)
    zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
    zblok.name = "skull"
    zblok.fade = rF32(0.3, 0.6)
    zblok.color = ranGrey()
    zblok.img = render.Skulls[rInt(0, len(render.Skulls))]
    if canadd {
      level.Level[0].etc = append(level.Level[0].etc, zblok)
      num--
    }
  ...
  makeInnerBloks(core, level)
  makemovebloks(level, render)
  makeBlades(level, render.Blades)
  makeTurrets(level, render, core)
  makeSpears(level, render)
  makeSpikes(level, render, core)

  //REMOVE BLOKS COLLIDING WITH BOSS REC
  checkrec := rl.NewRectangle(core.Cnt.X-bsU4, level.LevRecInner.Y+bsU2, bsU8, bsU8)
  numcollis := 0
  found := false
  for i := 0; i < len(level.Level[0].innerBloks); i++ {
    if rl.CheckCollisionRecs(checkrec, level.Level[0].innerBloks[i].rec) {
      numcollis = i
      found = true
    }
  }
  if found {
    level.Level[0].innerBloks = remBlok(level.Level[0].innerBloks, numcollis)
  }

  cleanLevel(level, core)

}

func makelevel(level *LevelState, shop *ShopState, enemies *EnemyState, core CoreState, render RenderState) { //MARK:MAKE LEVEL
  ...
  rec := rl.NewRectangle(core.Cnt.X-mapRecSize/2, core.Cnt.Y-mapRecSize/2, mapRecSize, mapRecSize)
  level.LevMap = append(level.LevMap, rec)
  numRooms--
  countbreak := 0
  for numRooms > 0 {

    choose := level.LevMap[0]
    if len(level.LevMap) > 1 {
      choose = level.LevMap[rInt(0, len(level.LevMap))]
    }
    ...
    canadd := true
    for a := 0; a < len(level.LevMap); a++ {
      if rl.CheckCollisionPointRec(checkV2, level.LevMap[a]) {
        canadd = false
      }
    }
  ...
  for a := 0; a < len(level.LevMap); a++ {
    ...
    checkV2 := rl.NewVector2(level.LevMap[a].X+level.LevMap[a].Width/2, level.LevMap[a].Y+level.LevMap[a].Width/2)
    ocheckV2 := checkV2
    checkV2.Y -= level.LevMap[a].Width
    for b := 0; b < len(level.LevMap); b++ {
      if a != b {
        if rl.CheckCollisionPointRec(checkV2, level.LevMap[b]) {
          level.Level[a].doorSides = append(level.Level[a].doorSides, 1)
          level.Level[a].nextRooms = append(level.Level[a].nextRooms, b)
        }
      }
    }
    checkV2 = ocheckV2
    checkV2.X += level.LevMap[a].Width
    for b := 0; b < len(level.LevMap); b++ {
      if a != b {
        if rl.CheckCollisionPointRec(checkV2, level.LevMap[b]) {
          level.Level[a].doorSides = append(level.Level[a].doorSides, 2)
          level.Level[a].nextRooms = append(level.Level[a].nextRooms, b)
        }
      }
    }
    ...
    for b := 0; b < len(level.LevMap); b++ {
      if a != b {
        if rl.CheckCollisionPointRec(checkV2, level.LevMap[b]) {
          level.Level[a].doorSides = append(level.Level[a].doorSides, 3)
          level.Level[a].nextRooms = append(level.Level[a].nextRooms, b)
        }
      }
    }
    ...
    for b := 0; b < len(level.LevMap); b++ {
      if a != b {
        if rl.CheckCollisionPointRec(checkV2, level.LevMap[b]) {
          level.Level[a].doorSides = append(level.Level[a].doorSides, 4)
          level.Level[a].nextRooms = append(level.Level[a].nextRooms, b)
        }
      }
    }
  ...
  for a := 0; a < len(level.Level); a++ {
    ...
    for b := 0; b < len(level.Level[a].doorSides); b++ {

      if level.Level[a].doorSides[b] == 1 {
        checkV2 := rl.NewVector2(level.LevX+level.LevW/2, level.LevY+level.BorderWallBlokSiz/2)
        checkV2L1 := checkV2
        checkV2L1.X -= level.BorderWallBlokSiz
        checkV2L2 := checkV2L1
        checkV2L2.X -= level.BorderWallBlokSiz
        checkV2R1 := checkV2
        checkV2R1.X += level.BorderWallBlokSiz
        checkV2R2 := checkV2R1
        checkV2R2.X += level.BorderWallBlokSiz

        exitRec := rl.NewRectangle(checkV2L2.X-level.BorderWallBlokSiz/2, checkV2L2.Y-level.BorderWallBlokSiz/2, level.BorderWallBlokSiz*5, bsU5)
        level.Level[a].doorExitRecs = append(level.Level[a].doorExitRecs, exitRec)

        for c := 0; c < len(level.Level[a].walls); c++ {
          if rl.CheckCollisionPointRec(checkV2, level.Level[a].walls[c].rec) {
            level.Level[a].walls = remBlok(level.Level[a].walls, c)
          }
        }
        for c := 0; c < len(level.Level[a].walls); c++ {
          if rl.CheckCollisionPointRec(checkV2L1, level.Level[a].walls[c].rec) {
            level.Level[a].walls = remBlok(level.Level[a].walls, c)
          }
        }
        for c := 0; c < len(level.Level[a].walls); c++ {
          if rl.CheckCollisionPointRec(checkV2L2, level.Level[a].walls[c].rec) {
            level.Level[a].walls = remBlok(level.Level[a].walls, c)
          }
        }
        for c := 0; c < len(level.Level[a].walls); c++ {
          if rl.CheckCollisionPointRec(checkV2R1, level.Level[a].walls[c].rec) {
            level.Level[a].walls = remBlok(level.Level[a].walls, c)
          }
        }
        for c := 0; c < len(level.Level[a].walls); c++ {
          if rl.CheckCollisionPointRec(checkV2R2, level.Level[a].walls[c].rec) {
            level.Level[a].walls = remBlok(level.Level[a].walls, c)
          }
        }
      }
      if level.Level[a].doorSides[b] == 2 {
        checkV2 := rl.NewVector2(level.LevX+level.LevW-level.BorderWallBlokSiz/2, level.LevY+level.LevW/2)
        checkV2L1 := checkV2
        checkV2L1.Y -= level.BorderWallBlokSiz
        checkV2L2 := checkV2L1
        checkV2L2.Y -= level.BorderWallBlokSiz
        checkV2R1 := checkV2
        checkV2R1.Y += level.BorderWallBlokSiz
        checkV2R2 := checkV2R1
        checkV2R2.Y += level.BorderWallBlokSiz

        exitRec := rl.NewRectangle((checkV2L2.X+level.BorderWallBlokSiz/2)-bsU5, checkV2L2.Y-level.BorderWallBlokSiz/2, bsU5, level.BorderWallBlokSiz*5)
        level.Level[a].doorExitRecs = append(level.Level[a].doorExitRecs, exitRec)
      ...
      if level.Level[a].doorSides[b] == 3 {
        checkV2 := rl.NewVector2(level.LevX+level.LevW/2, level.LevY+level.LevW-level.BorderWallBlokSiz/2)
        checkV2L1 := checkV2
        checkV2L1.X -= level.BorderWallBlokSiz
        checkV2L2 := checkV2L1
        checkV2L2.X -= level.BorderWallBlokSiz
        ...
        exitRec := rl.NewRectangle(checkV2L2.X-level.BorderWallBlokSiz/2, (checkV2L2.Y+level.BorderWallBlokSiz/2)-bsU5, level.BorderWallBlokSiz*5, bsU5)
        level.Level[a].doorExitRecs = append(level.Level[a].doorExitRecs, exitRec)
      ...
      if level.Level[a].doorSides[b] == 4 {
        checkV2 := rl.NewVector2(level.LevX+level.BorderWallBlokSiz/2, level.LevY+level.LevW/2)
        checkV2L1 := checkV2
        checkV2L1.Y -= level.BorderWallBlokSiz
        checkV2L2 := checkV2L1
        checkV2L2.Y -= level.BorderWallBlokSiz
        ...
        exitRec := rl.NewRectangle((checkV2L2.X - level.BorderWallBlokSiz/2), checkV2L2.Y-level.BorderWallBlokSiz/2, bsU5, level.BorderWallBlokSiz*5)
        level.Level[a].doorExitRecs = append(level.Level[a].doorExitRecs, exitRec)
  ...
  level.Level[0].visited = true

  makeInnerBloks(core, level)
  makemovebloks(level, render)
  makeStatues(level, render.Statues)
  makeshop(shop, level, render, core.Cnt)
  makeetc(level, render)
  ...
  makeenemies(level, enemies, render, core.Fps)

  makeExit(level, render)

  cleanLevel(level, core)

}

func makenewlevel(level *LevelState, player *PlayerState, fx *FXState, companions CompanionState, shop *ShopState, enemies *EnemyState, core CoreState, render RenderState) { //MARK:MAKE NEW LEVEL

  level.NextLevelScreen = true
  level.Levelnum++

  for a := 0; a < len(level.Level); a++ {
    level.Level[a].etc = nil
    level.Level[a].enemies = nil
    level.Level[a].doorExitRecs = nil
    level.Level[a].doorSides = nil
    level.Level[a].floor = nil
    level.Level[a].innerBloks = nil
    level.Level[a].movBloks = nil
    level.Level[a].nextRooms = nil
    level.Level[a].spikes = nil
    level.Level[a].visited = false
    level.Level[a].walls = nil
  }
  ...
  if level.Levelnum == 6 {
    player.Mods.planty = false
    player.Mods.carrot = false
    player.Mods.alien = false
    player.Mods.vine = false
    player.Mods.airstrike = false
    makeendlevel(level, core, render)
  } else {
    makelevel(level, shop, enemies, core, render)
  }
  level.NextlevelT = core.Fps

  player.Pl.cnt = core.Cnt
  cntCompanion := player.Pl.cnt

  if player.Mods.carrot {
    companions.MrCarrot.rec = rl.NewRectangle(cntCompanion.X-companions.MrCarrot.rec.Width/2, cntCompanion.Y-companions.MrCarrot.rec.Width/2, companions.MrCarrot.rec.Width, companions.MrCarrot.rec.Width)
  }
  if player.Mods.alien {
    companions.MrAlien.rec = rl.NewRectangle(cntCompanion.X-companions.MrAlien.rec.Width/2, cntCompanion.Y-companions.MrAlien.rec.Width/2, companions.MrAlien.rec.Width, companions.MrAlien.rec.Width)
  }
  if player.Mods.planty {
    companions.MrPlanty.rec = rl.NewRectangle(cntCompanion.X-companions.MrPlanty.rec.Width/2, cntCompanion.Y-companions.MrPlanty.rec.Width/2, companions.MrPlanty.rec.Width, companions.MrPlanty.rec.Width)
  }
...
func makeInnerBloks(core CoreState, level *LevelState) { //MARK: MAKE INNER BLOKS
	for a := 0; a < len(level.Level); a++ {
		num := rInt(3, 7)
		for num > 0 {
			countbreak := 100
			for {
				siz := rF32(bsU4, bsU8)
				zblok := xblok{}
				tl := findRanRecLoc(level, siz, siz, a)
				zblok.rec = rl.NewRectangle(tl.X, tl.Y, siz, siz)
				zblok.img = level.WallT
				switch level.Levelnum {
				case 1:
					zblok.color = ranBlue()
				case 2:
				...
				zblok.solid = true
				canadd := true
				for b := 0; b < len(level.Level[a].innerBloks); b++ {
					if rl.CheckCollisionRecs(zblok.rec, level.Level[a].innerBloks[b].rec) {
						canadd = false
					}
					if a == 0 {
						checkrec := rl.NewRectangle(core.Cnt.X-bsU4, core.Cnt.Y-bsU4, bsU8, bsU8)
				...
				countbreak--
				if canadd || countbreak == 0 {
					level.Level[a].innerBloks = append(level.Level[a].innerBloks, zblok)
					break
				}
			}
			num--
...
func makeAirstrike(fx *FXState, level *LevelState, core CoreState) { //MARK:MAKE AIR STRIKE
	fx.AirstrikeV2 = nil
	fx.AirstrikeDir = rInt(1, 5)
	switch fx.AirstrikeDir {
	case 1:
		v2 := rl.NewVector2(rF32(level.LevRecInner.X+bsU4, level.LevRecInner.X+level.LevRecInner.Width-bsU8), level.LevRecInner.Y-bsU4)
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		v2.Y -= bsU4
		v2.X -= bsU4
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		v2.X += bsU8
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
	case 2:
		v2 := rl.NewVector2(level.LevRecInner.X+level.LevRecInner.Width+bsU4, rF32(level.LevRecInner.Y+bsU4, level.LevRecInner.Y+level.LevRecInner.Width-bsU8))
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		v2.Y -= bsU4
		v2.X += bsU4
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		...
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
	case 3:
		v2 := rl.NewVector2(rF32(level.LevRecInner.X+bsU4, level.LevRecInner.X+level.LevRecInner.Width-bsU8), level.LevRecInner.Y+level.LevRecInner.Width+bsU4)
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		v2.Y += bsU4
		v2.X -= bsU4
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		...
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
	case 4:
		v2 := rl.NewVector2(level.LevRecInner.X-bsU4, rF32(level.LevRecInner.Y+bsU4, level.LevRecInner.Y+level.LevRecInner.Width-bsU8))
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
		v2.Y -= bsU4
		v2.X -= bsU4
		fx.AirstrikeV2 = append(fx.AirstrikeV2, v2)
	...
	fx.AirstrikebombT = rI32(int(core.Fps/4), int(core.Fps*2))
	fx.AirstrikeOn = true
}

func makeFish(fx *FXState, core CoreState, render RenderState) { //MARK:MAKE FISH

  fx.FishV2 = rl.Vector2{}
  fx.Fish2V2 = rl.Vector2{}

  fx.FishSiz = rF32(bsU4, bsU7)
  fx.FishSiz2 = rF32(bsU4, bsU7)

  fx.FishV2.X = -fx.FishSiz
  fx.Fish2V2.X = core.ScrWF32 + fx.FishSiz2

  fx.FishV2.Y = rF32(core.ScrHF32/3, core.ScrHF32)
  fx.Fish2V2.Y = rF32(core.ScrHF32/3, core.ScrHF32)

  fx.FishRec = rl.NewRectangle(
    fx.FishV2.X,
    fx.FishV2.Y,
    fx.FishSiz,
    fx.FishSiz)
  fx.FishRec2 = rl.NewRectangle(
    fx.Fish2V2.X,
    fx.Fish2V2.Y,
    fx.FishSiz2,
    fx.FishSiz2)
...
func makePlayer(player *PlayerState, render *RenderState, core *CoreState) { //MARK:MAKE PLAYER
  player.Pl.atkDMG = 1
  player.Pl.cnt = core.Cnt
  player.Pl.hp = 5
  player.Pl.hpmax = 5
  player.Pl.vel = 4
  player.Pl.siz = 72
  player.Pl.rec = rl.NewRectangle(player.Pl.cnt.X-player.Pl.siz/2, player.Pl.cnt.Y-player.Pl.siz/2, player.Pl.siz, player.Pl.siz)
  player.Pl.crec = player.Pl.rec
  player.Pl.arec = player.Pl.rec
  player.Pl.atkrec = player.Pl.rec
  player.Pl.atkrec.X -= bsU
  player.Pl.atkrec.Y -= bsU
  player.Pl.atkrec.Width += bsU2
  player.Pl.atkrec.Height += bsU2
  player.Pl.crec.X += player.Pl.crec.Width / 3
  player.Pl.crec.Y += player.Pl.crec.Height / 3
  player.Pl.crec.Width = player.Pl.crec.Width / 3
  player.Pl.crec.Height = player.Pl.crec.Height / 2
  player.Pl.framesAtk = 6
  player.Pl.framesWalk = 8
  player.Pl.sizImg = 32
  player.Pl.img = render.Knight[0]
  player.Pl.ori = rl.NewVector2(player.Pl.rec.Width/2, player.Pl.rec.Height/2)
  player.Pl.orbimg1 = render.Orbitalanim.recTL
  player.Pl.orbimg2 = player.Pl.orbimg1
  player.Max.axe = 10
  player.Max.fireball = 8
  player.Max.bounce = 4
  player.Max.key = 3
  player.Max.apple = 3
  player.Max.firetrail = 3
  player.Max.hppotion = 3
  player.Max.coffee = 5
  player.Max.atkrange = 3
  player.Max.atkdmg = 2
  player.Max.orbital = 2
  player.Max.hpring = 3
  player.Max.armor = 3
  player.Max.cherry = 99
  player.Max.cake = 99
}

func makeEnemyTypes(enemies *EnemyState) { //MARK: MAKE ENEMY TYPES

  siz := bsU4

  enemies.EnSpikes.img = rl.NewRectangle(0, 404, 44, 44)
  enemies.EnSpikes.fade = 1
  enemies.EnSpikes.col = rl.White
  enemies.EnSpikes.xImg = 0
  enemies.EnSpikes.frameNum = 15
  enemies.EnSpikes.vel = bsU / 5
  enemies.EnSpikes.name = "spikehog"
  enemies.EnSpikes.hp = 5
  enemies.EnSpikes.hpmax = enemies.EnSpikes.hp
  enemies.EnSpikes.rec = rl.NewRectangle(0, 0, siz, siz)

  enemies.EnGhost.imgl = rl.NewRectangle(4, 506, 44, 44)
  enemies.EnGhost.imgr = rl.NewRectangle(454, 506, 44, 44)
  enemies.EnGhost.img = enemies.EnGhost.imgr
  enemies.EnGhost.col = rl.White
  enemies.EnGhost.fade = 1
  enemies.EnGhost.xImg = 4
  enemies.EnGhost.xImg2 = 454
  enemies.EnGhost.frameNum = 9
  enemies.EnGhost.vel = bsU / 5
  enemies.EnGhost.name = "ghost"
  enemies.EnGhost.hp = 4
  enemies.EnGhost.hpmax = enemies.EnGhost.hp
  enemies.EnGhost.rec = rl.NewRectangle(0, 0, siz, siz)

  enemies.EnSlime.imgl = rl.NewRectangle(4, 556, 44, 44)
  enemies.EnSlime.imgr = rl.NewRectangle(458, 556, 44, 44)
  enemies.EnSlime.img = enemies.EnSlime.imgr
  enemies.EnSlime.col = rl.White
  enemies.EnSlime.fade = 1
  enemies.EnSlime.xImg = 4
  enemies.EnSlime.xImg2 = 458
  enemies.EnSlime.frameNum = 9
  enemies.EnSlime.vel = bsU / 5
  enemies.EnSlime.name = "slime"
  enemies.EnSlime.hp = 5
  enemies.EnSlime.hpmax = enemies.EnSlime.hp
  enemies.EnSlime.rec = rl.NewRectangle(0, 0, siz, siz)

  enemies.EnRock.imgl = rl.NewRectangle(4, 608, 22, 22)
  enemies.EnRock.imgr = rl.NewRectangle(318, 608, 22, 22)
  enemies.EnRock.img = enemies.EnRock.imgr
  enemies.EnRock.col = rl.White
  enemies.EnRock.fade = 1
  enemies.EnRock.xImg = 4
  enemies.EnRock.xImg2 = 318
  enemies.EnRock.frameNum = 13
  enemies.EnRock.vel = bsU / 4
  enemies.EnRock.name = "rock"
  enemies.EnRock.hp = 3
  enemies.EnRock.hpmax = enemies.EnRock.hp
  enemies.EnRock.rec = rl.NewRectangle(0, 0, bsU3, bsU3)

  enemies.EnMushroom.imgl = rl.NewRectangle(4, 634, 32, 32)
  enemies.EnMushroom.imgr = rl.NewRectangle(4, 668, 32, 32)
  enemies.EnMushroom.img = enemies.EnMushroom.imgr
  enemies.EnMushroom.col = rl.White
  enemies.EnMushroom.fade = 1
  enemies.EnMushroom.xImg = 4
  enemies.EnMushroom.xImg2 = 4
  enemies.EnMushroom.frameNum = 13
  enemies.EnMushroom.vel = bsU / 5
  enemies.EnMushroom.name = "mushroom"
  enemies.EnMushroom.hp = 5
  enemies.EnMushroom.hpmax = enemies.EnMushroom.hp
  enemies.EnMushroom.rec = rl.NewRectangle(0, 0, siz, siz)

}

func makeenemies(level *LevelState, enemies *EnemyState, render RenderState, fps int32) { //MARK:MAKE ENEMIES

  makeEnemyTypes(enemies)
  ...
  for a := 0; a < len(level.Level); a++ {
    ...
    if level.Levelnum > 4 {
      if flipcoin() { //MUSHROOM
        zen = xenemy{}
        zen = enemies.EnMushroom
        zen.T1 = rI32(int(fps*2), int(fps*5))
        canadd := true
        zen.rec, canadd = findRecPos(level, enemies.EnMushroom.rec.Width, a)
        zen.crec = zen.rec
        ...
        zen.velY = rF32(-zen.vel, zen.vel)
        if canadd {
          level.Level[a].enemies = append(level.Level[a].enemies, zen)
        }
      }
      if level.Hardcore {
        if flipcoin() { //MUSHROOM
    ...
    if level.Levelnum > 3 {
      ...
      if flipcoin() { //GHOST
        ...
        zen.fly = true
        canadd := true
        zen.rec, canadd = findRecPos(level, enemies.EnGhost.rec.Width, a)
        zen.crec = zen.rec
        zen.crec.Y += zen.crec.Height / 3
        zen.crec.Height -= zen.crec.Height / 3
    ...
    if level.Levelnum > 1 {
      ...
      if flipcoin() { //SLIME
        ...
        canadd := true
        zen.rec, canadd = findRecPos(level, enemies.EnSlime.rec.Width, a)
        zen.crec = zen.rec
        zen.crec.Y += zen.crec.Height / 3
        zen.crec.Height -= zen.crec.Height / 3
        zen.velX = rF32(-zen.vel, zen.vel)
    ...
    if flipcoin() { //ROCK
      ...
      zen.T1 = rI32(int(fps*2), int(fps*5))
      canadd := true
      zen.rec, canadd = findRecPos(level, enemies.EnRock.rec.Width, a)
      zen.crec = zen.rec
      zen.crec.Y += zen.crec.Height / 3
      zen.crec.Height -= zen.crec.Height / 3
      zen.velX = rF32(-zen.vel, zen.vel)
      ...
      zen = xenemy{}
      zen = enemies.EnSpikes
      canadd := true
      zen.rec, canadd = findRecPos(level, enemies.EnSpikes.rec.Width, a)
      zen.crec = zen.rec
      zen.crec.Height = zen.crec.Height / 2
    ...
    if flipcoin() {
      ...
      zen.velX = rF32(-zen.vel, zen.vel)
      zen.velY = rF32(-zen.vel, zen.vel)
      zen.img = render.Bats[rInt(0, len(render.Bats))]
      zen.fade = 1
      zen.col = rl.White
      zen.frameNum = 3
      zen.xImg = zen.img.X
      canadd := true
      zen.rec, canadd = findRecPos(level, siz, a)
    ...
    if flipcoin() {
      ...
      zen.crec = zen.rec
      if canadd {
        if zen.rec.X > level.LevRecInner.X && zen.rec.Y > level.LevRecInner.Y {
          level.Level[a].enemies = append(level.Level[a].enemies, zen)
        }
      }
...
func makeGasCloud(render *RenderState, level *LevelState, cnt rl.Vector2) { //MARK:MAKE GAS CLOUD
  siz := bsU10
  zblok := xblok{}
  zblok.fade = 1
  zblok.cnt = cnt
  ...
  zblok.onoff = true
  zblok.color = ranGreen()
  zblok.img = render.Posiongas.recTL
  zblok.vel = bsU / 4
...
func makeSwitchArrows(level *LevelState, enemies *EnemyState, render *RenderState) { //MARK:MAKE SWITCH ARROWS
	choose := rInt(1, 5)
	switch choose {
	case 1:
		x := level.LevRecInner.X + bsU
		y := level.LevRecInner.Y
		for {
			makeProjectileEnemy(enemies, render,  2, rl.NewVector2(x+bsU, y+bsU))
			x += bsU2
			if x > level.LevRecInner.X+level.LevRecInner.Width-bsU2 {
				break
			}
		}
	case 2:
		x := level.LevRecInner.X + level.LevRecInner.Width - bsU2
		y := level.LevRecInner.Y + bsU
		for {
			makeProjectileEnemy(enemies, render, 5, rl.NewVector2(x+bsU, y+bsU))
			y += bsU2
			if y > level.LevRecInner.Y+level.LevRecInner.Width-bsU2 {
				break
			}
		}
	case 3:
		x := level.LevRecInner.X + bsU
		y := level.LevRecInner.Y + level.LevRecInner.Width - bsU2
		for {
			makeProjectileEnemy(enemies,render, 4, rl.NewVector2(x+bsU, y+bsU))
			x += bsU2
			if x > level.LevRecInner.X+level.LevRecInner.Width-bsU2 {
				break
			}
	...
	case 4:
		x := level.LevRecInner.X
		y := level.LevRecInner.Y + bsU
		for {
			makeProjectileEnemy(enemies,render, 3, rl.NewVector2(x+bsU, y+bsU))
			y += bsU2
			if y > level.LevRecInner.Y+level.LevRecInner.Width-bsU2 {
				break
			}
...
func makeExit(level *LevelState, render RenderState) { //MARK:MAKE EXIT
  for {
    level.ExitRoomNum = rInt(1, len(level.Level))
    if level.ExitRoomNum != level.ShopRoomNum {
      level.Level[level.ExitRoomNum].exit = true
      break
    }
  }
  zblok := makeBlokGenNoRecNoCntr()
  zblok.img = render.Etc[53]
  zblok.name = "exit"
  zblok.color = ranBrown()
  siz := bsU3
  for {
    canadd := true
    zblok.rec, canadd = findRecPoswithSpacing(level, siz, bsU2, level.ExitRoomNum)
    if canadd {
      break
    }
  }
  level.Level[level.ExitRoomNum].etc = append(level.Level[level.ExitRoomNum].etc, zblok)
}

func makeStatues(lvl *LevelState, statues []rl.Rectangle) { //MARK:MAKE STATUES
  for a := 0; a < len(gs.Level.Level); a++ {
    if roll6() > 4 {
      zblok := makeBlokGenNoRecNoCntr()
      siz := rF32(bsU3, bsU5)
      found := true
      zblok.rec, found = findRecPoswithSpacing(lvl, siz, bsU2, a)
      zblok.img = statues[rInt(0, len(statues))]
      zblok.name = "statue"
      zblok.solid = true

      if found {
        lvl.Level[a].etc = append(lvl.Level[a].etc, zblok)
      }
    }
  }
}

func makeSpears(level *LevelState, render RenderState) { //MARK:MAKE SPEARS
	for a := 0; a < len(level.Level); a++ {
		zblok := makeBlokGenNoRecNoCntr()
		multi := float32(2)
		choose := rInt(1, 5)
		...
		case 1:
			zblok.ro = 180
			zblok.rec = rl.NewRectangle(level.LevRecInner.X, level.LevRecInner.Y, multi*render.Spear.recTL.Width, multi*render.Spear.recTL.Height)
			for {
				zblok.rec.X += rF32(0, level.LevRecInner.Width-bsU2)
				if checkInnerBloksExits(*level, a, zblok.rec) {
					break
				}
			}
			zblok.drec = zblok.rec
		...
		case 2:
			zblok.ro = 270
			zblok.rec = rl.NewRectangle(level.LevRecInner.X+level.LevRecInner.Width-bsU-(multi*render.Spear.recTL.Height)/2, level.LevRecInner.Y+bsU-(multi*render.Spear.recTL.Height)/2, multi*render.Spear.recTL.Width, multi*render.Spear.recTL.Height)
			for {
				zblok.rec.Y += rF32(0, level.LevRecInner.Width-bsU2)
				if checkInnerBloksExits(*level, a, zblok.rec) {
					break
				}
			}
			...
			zblok.drec.X += zblok.drec.Width / 2
			zblok.drec.Y += zblok.drec.Height / 2
			zblok.crec = rl.NewRectangle(level.LevRecInner.X+level.LevRecInner.Width-multi*render.Spear.recTL.Height, (zblok.rec.Y+zblok.rec.Height/2)-bsU, multi*render.Spear.recTL.Height, multi*render.Spear.recTL.Width)
			zblok.ori = rl.NewVector2(zblok.drec.Width/2, zblok.drec.Height/2)
		case 3:
			zblok.ro = 0
			zblok.rec = rl.NewRectangle(level.LevRecInner.X, level.LevRecInner.Y+level.LevRecInner.Height-(multi*render.Spear.recTL.Height), multi*render.Spear.recTL.Width, multi*render.Spear.recTL.Height)
			for {
				zblok.rec.X += rF32(0, level.LevRecInner.Width-bsU2)
				if checkInnerBloksExits(*level, a, zblok.rec) {
					break
		...
		case 4:
			zblok.ro = 90
			zblok.rec = rl.NewRectangle(level.LevRecInner.X+bsU3, level.LevRecInner.Y+bsU-(multi*render.Spear.recTL.Height)/2, multi*render.Spear.recTL.Width, multi*render.Spear.recTL.Height)
			for {
				zblok.rec.Y += rF32(0, level.LevRecInner.Width-bsU2)
				if checkInnerBloksExits(*level, a, zblok.rec) {
					break
			...
			zblok.drec.X += zblok.drec.Width / 2
			zblok.drec.Y += zblok.drec.Height / 2
			zblok.crec = rl.NewRectangle(zblok.rec.X-bsU3, (zblok.rec.Y+zblok.rec.Height/2)-bsU, multi*render.Spear.recTL.Height, multi*render.Spear.recTL.Width)
			zblok.ori = rl.NewVector2(zblok.drec.Width/2, zblok.drec.Height/2)
		}
		zblok.img = render.Spear.recTL
		zblok.name = "spear"
		if zblok.rec.X > level.LevRecInner.X && zblok.rec.X < level.LevRecInner.Width+level.LevRecInner.X && zblok.rec.Y > level.LevRecInner.Y && zblok.rec.Y < level.LevRecInner.Y+level.LevRecInner.Width {
			level.Level[a].etc = append(level.Level[a].etc, zblok)
		}
	}
}

func makeBlades(lvl *LevelState, blades xanim) { //MARK:MAKE BLADES
  for a := 0; a < len(lvl.Level); a++ {
    if flipcoin() {
      siz := rF32(bsU2, bsU4)
      zblok := makeBlokGenNoRecNoCntr()
      found := true
      zblok.rec, found = findRecPos(&gs.Level, siz, a)
...
func makeetc(level *LevelState, render RenderState) { //MARK:MAKE ETC
  ...
  for a := 0; a < len(level.Level); a++ {
    ...
    if roll6() == 6 {
      zblok := makeBlokGenNoRecNoCntr()
      canadd := true
      zblok.rec, canadd = findRecPoswithSpacing(level, bsU2, bsU/2, a)
      zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
      zblok.drec = zblok.rec
      zblok.drec.X += zblok.rec.Width / 2
      zblok.drec.Y += zblok.rec.Height / 2
    ...
    if roll6() == 6 {
      ...
      zblok := makeBlokGenNoRecNoCntr()
      canadd := true
      zblok.rec, canadd = findRecPoswithSpacing(level, siz, bsU/2, a)
      zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
      zblok.name = "chest"
    ...
    for {
      ...
      siz := rF32((bsU/4)*3, bsU+bsU/2)
      canadd := true
      zblok.rec, canadd = findRecPos(level, siz, a)
      zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
      zblok.name = "skull"
      zblok.fade = rF32(0.3, 0.6)
      zblok.color = ranGrey()
    ...
    for {
      ...
      zblok.fade = rF32(0.5, 0.8)
      zblok.color = rl.White
      zblok.img = render.Candles[rInt(0, len(render.Candles))]
      if canadd {
        level.Level[a].etc = append(level.Level[a].etc, zblok)
        num--
      }
    ...
    for {
      ...
      zblok.color = ranBrown()
      zblok.img = render.Signs[rInt(0, len(render.Signs))]
      if canadd {
        level.Level[a].etc = append(level.Level[a].etc, zblok)
        num--
      }
    ...
    for {
      ...
      zblok.fade = rF32(0.3, 0.6)
      zblok.color = ranGreen()
      zblok.img = render.Plants[rInt(0, len(render.Plants))]
      if canadd {
        level.Level[a].etc = append(level.Level[a].etc, zblok)
        num--
      }
    ...
    for {
      ...
      zblok.fade = rF32(0.3, 0.6)
      zblok.color = rl.White
      zblok.img = render.Mushrooms[rInt(0, len(render.Mushrooms))]
      if canadd {
        level.Level[a].etc = append(level.Level[a].etc, zblok)
        num--
      }
    ...
    if roll6() > 2 {
      zblok := makeBlokGenNoRecNoCntr()
      canadd := true
      zblok.rec, canadd = findRecPoswithSpacing(level, bsU+bsU/2, bsU/4, a)
      zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
      zblok.solid = true
      zblok.name = "powerupBlok"
      zblok.color = brightYellow()
      zblok.img = render.Etc[27]
      if canadd {
        level.Level[a].etc = append(level.Level[a].etc, zblok)
    ...
    if flipcoin() {
      ...
      for num > 0 {
        ...
        switch choose {
        case 1:
          zblok.rec.Y = level.LevRecInner.Y
          for {
            zblok.rec.X = level.LevRecInner.X + rF32(0, level.LevRecInner.Width-bsU2)
            canadd := true
            for b := 0; b < len(level.Level[a].doorExitRecs); b++ {
              if rl.CheckCollisionRecs(zblok.rec, level.Level[a].doorExitRecs[b]) {
                canadd = false
              }
            }
            if canadd {
          ...
          zblok.slideDIR = 3
        case 2:
          zblok.rec.X = level.LevRecInner.X + level.LevRecInner.Width - siz
          for {
            zblok.rec.Y = level.LevRecInner.Y + rF32(0, level.LevRecInner.Width-bsU2)
            canadd := true
            for b := 0; b < len(level.Level[a].doorExitRecs); b++ {
              if rl.CheckCollisionRecs(zblok.rec, level.Level[a].doorExitRecs[b]) {
                canadd = false
          ...
          zblok.slideDIR = 4
        case 3:
          zblok.rec.Y = level.LevRecInner.Y + level.LevRecInner.Width - siz
          for {
            zblok.rec.X = level.LevRecInner.X + rF32(0, level.LevRecInner.Width-bsU2)
            canadd := true
            for b := 0; b < len(level.Level[a].doorExitRecs); b++ {
          ...
          zblok.slideDIR = 1
        case 4:
          zblok.rec.X = level.LevRecInner.X
          for {
            zblok.rec.Y = level.LevRecInner.Y + rF32(0, level.LevRecInner.Width-bsU2)
            canadd := true
            for b := 0; b < len(level.Level[a].doorExitRecs); b++ {
    ...
    if flipcoin() {
      ...
      canadd := true
      zblok.rec, canadd = findRecPoswithSpacing(level, siz, bsU2, a)
      zblok.cnt = rl.NewVector2(zblok.rec.X+zblok.rec.Width/2, zblok.rec.Y+zblok.rec.Width/2)
      zblok.solid = true
...
func makeSnow(core CoreState, level *LevelState, render RenderState, fx *FXState) { //MARK:MAKE SNOW
  num := rInt(50, 100)
  for {
    x := rF32(0, core.ScrWF32)
    y := level.LevY - rF32(bsU, core.ScrHF32)
    siz := rF32(bsU, bsU3)
    zimg := ximg{}
    ...
    switch choose {
    case 1:
      zimg.img = render.Etc[15]
    case 2:
      zimg.img = render.Etc[16]
    case 3:
      zimg.img = render.Etc[17]
    }
    zimg.ori = rl.NewVector2(zimg.rec.Width/2, zimg.rec.Height/2)
    zimg.ro = rF32(0, 360)
    zimg.col = rl.White
    zimg.fade = rF32(0.3, 0.7)
    fx.Snow = append(fx.Snow, zimg)
    num--
    if num == 0 {
      break
    }
...
func makeSpikes(level *LevelState, render RenderState, core CoreState) { //MARK: MAKE SPIKES
	siz := bsU2
	zblok := xblok{}
	zblok.name = "spikes"
	zblok.fade = 1
	zblok.img = render.Spikes.recTL
	for a := 0; a < len(level.Level); a++ {
		if flipcoin() {
			zblok.cnt = findRanCntV2(core.Cnt, level.LevW)
			num := rInt(10, 30)
			for num > 0 {
				zblok.color = ranBlue()
				zblok.rec = rl.NewRectangle(zblok.cnt.X-siz/2, zblok.cnt.Y-siz/2, siz, siz)
				...
				v4 := v1
				v4.Y += siz
				if rl.CheckCollisionPointRec(v1, level.LevRecInner) && rl.CheckCollisionPointRec(v2, level.LevRecInner) && rl.CheckCollisionPointRec(v3, level.LevRecInner) && rl.CheckCollisionPointRec(v4, level.LevRecInner) {
					level.Level[a].spikes = append(level.Level[a].spikes, zblok)
				}
				choose := rInt(1, 5)
				switch choose {
				case 1:
...
func makeTurrets(level *LevelState, render RenderState, core CoreState) { //MARK: MAKE TURRETS
	siz := bsU2
	zblok := xblok{}
	zblok.name = "turret"
	zblok.img = render.Etc[18]
	zblok.solid = true
	zblok.fade = 1
	zblok.onoff = true
	for a := 0; a < len(level.Level); a++ {
...
func makeBlokGenRandom(lvl LevelState, size float32) xblok { //MARK:MAKE GENERIC BLOCK RANDOM POSITION
  zblok := xblok{}
  zblok.fade = 1
  zblok.color = ranCol()

  x := lvl.LevRecInner.X + bsU
  y := lvl.LevRecInner.Y + bsU
  x = rF32(x, x+lvl.LevRecInner.Width-bsU3)
  y = rF32(y, y+lvl.LevRecInner.Width-bsU3)
...
func makemovebloks(level *LevelState, render RenderState) { //MARK:MAKE MOVE BLOKS

  zblok := xblok{}
  bloksiz := bsU2
  blokimg := render.Walltiles[rInt(0, len(render.Walltiles))]
  zblok.fade = 1
  ...
  for a := 0; a < len(level.Level); a++ {
    ...
    for b := 0; b < bloknum; b++ {
      ...
      for {
        ...
        canadd := true
        zblok.rec, canadd = findRecPos(level,bloksiz, a)
        zblok.cnt = makeCnt(zblok)
        ...
        if len(level.Level[a].movBloks) > 0 {
          for c := 0; c < len(level.Level[a].movBloks); c++ {
            if rl.CheckCollisionRecs(zblok.rec, level.Level[a].movBloks[c].rec) {
              canadd = false
            }
          }
        }
      ...
      if addtolevel {
        ...
        case 2: //UD
          ...
          zblok.movType = 2 //UD
          level.Level[a].movBloks = append(level.Level[a].movBloks, zblok)
        case 1: //LR
          zblok.velX = rF32(zblok.vel/4, zblok.vel)
          if roll12() == 12 {
            zblok.velY = rF32(zblok.vel/4, zblok.vel)
...
func makeCompanions(companions *CompanionState, core *CoreState, render *RenderState) { //MARK: MAKE COMPANIONS
	//MR CARROT
	siz := bsU3
	companions.MrCarrot.rec = rl.NewRectangle(core.Cnt.X, core.Cnt.Y, siz, siz)
	companions.MrCarrot.imgl = rl.NewRectangle(0, 248, 38, 38)
	companions.MrCarrot.imgr = rl.NewRectangle(228, 248, 38, 38)
	companions.MrCarrot.vel = bsU / 5
	companions.MrCarrot.velx = rF32(-companions.MrCarrot.vel, companions.MrCarrot.vel)
	companions.MrCarrot.vely = rF32(-companions.MrCarrot.vel, companions.MrCarrot.vel)
	companions.MrCarrot.hp = 5
	companions.MrCarrot.hpmax = companions.MrCarrot.hp
	companions.MrCarrot.frames = 5
	companions.MrCarrot.timer = core.Fps * rI32(1, 5)
	//MR PLANTY
	companions.MrPlanty.rec = rl.NewRectangle(core.Cnt.X, core.Cnt.Y, siz, siz)
	companions.MrPlanty.imgl = rl.NewRectangle(0, 454, 44, 44)
	companions.MrPlanty.imgr = rl.NewRectangle(352, 454, 44, 44)
	companions.MrPlanty.vel = bsU / 5
	companions.MrPlanty.velx = rF32(-companions.MrPlanty.vel, companions.MrPlanty.vel)
	companions.MrPlanty.vely = rF32(-companions.MrPlanty.vel, companions.MrPlanty.vel)
	companions.MrPlanty.hp = 5
	companions.MrPlanty.hpmax = companions.MrPlanty.hp
	companions.MrPlanty.frames = 7
	//ALIEN
	companions.MrAlien.rec = rl.NewRectangle(core.Cnt.X, core.Cnt.Y, siz, siz)
	companions.MrAlien.img = render.Alien[0]
	companions.MrAlien.vel = bsU / 5
	companions.MrAlien.velx = rF32(-companions.MrAlien.vel, companions.MrAlien.vel)
	companions.MrAlien.vely = rF32(-companions.MrAlien.vel, companions.MrAlien.vel)
	companions.MrAlien.hp = 5
	companions.MrAlien.hpmax = companions.MrAlien.hp
	companions.MrAlien.timer = core.Fps * rI32(3, 8)
}

func makeFxInitial(fx *FXState, core *CoreState) { //MARK:MAKE FX INITIAL
  num := 300
  for a := 0; a < num; a++ {
    siz := rF32(2, 5)
    rec := rl.NewRectangle(rF32(0, core.ScrWF32), rF32(-core.ScrHF32, -bsU), siz, siz)
    fx.Rain = append(fx.Rain, rec)
  }
  y := float32(-2)
  x := float32(0)
  change := float32(3)
  for {
    fx.ScanlineV2 = append(fx.ScanlineV2, rl.NewVector2(x, y))
    y += change
    if y >= core.ScrHF32+1 {
      break
    }
...
func makeBosses(core *CoreState, level *LevelState) { //MARK:MAKE BOSSES
  siz := bsU8
  zboss := xboss{}
  zboss.hp = 20
  zboss.timer = core.Fps * 4
  ...
  zboss.rec = rl.NewRectangle(core.Cnt.X-siz/2, level.LevRecInner.Y+bsU2, siz, siz)
  zboss.cnt = core.Cnt
  ...
  level.Bosses = append(level.Bosses, zboss)
...
func makeImgs(render *RenderState, fx *FXState, player *PlayerState) { //MARK:MAKE IMGS
	render.Etc = append(render.Etc, rl.NewRectangle(2, 36, 18, 18))       // 0 BARREL
	render.Etc = append(render.Etc, rl.NewRectangle(24, 36, 16, 16))      // 1 HP POTION
	render.Etc = append(render.Etc, rl.NewRectangle(42, 36, 16, 16))      // 2 PLAYER HP ICON
	render.Etc = append(render.Etc, rl.NewRectangle(60, 36, 14, 14))      // 3 THROWING AXE
	render.Etc = append(render.Etc, rl.NewRectangle(77, 36, 18, 18))      // 4 SANTA
	render.Etc = append(render.Etc, rl.NewRectangle(99, 36, 13, 13))      // 5 BOUNCE PROJECTILE
	render.Etc = append(render.Etc, rl.NewRectangle(115, 36, 16, 16))     // 6 ESCAPE VINE
	render.Etc = append(render.Etc, rl.NewRectangle(132, 35, 17, 17))     // 7 SKELETON KEY
	render.Etc = append(render.Etc, rl.NewRectangle(151, 35, 16, 16))     // 8 APPLE
	render.Etc = append(render.Etc, rl.NewRectangle(170, 36, 17, 17))     // 9 PLANT COMPANION
	render.Etc = append(render.Etc, rl.NewRectangle(192, 36, 18, 18))     // 10 MEDI KIT
	render.Etc = append(render.Etc, rl.NewRectangle(215, 38, 16, 16))     // 11 WALLET
	render.Etc = append(render.Etc, rl.NewRectangle(235, 38, 15, 15))     // 12 RECHARGE
	render.Etc = append(render.Etc, rl.NewRectangle(1082, 234, 32, 32))   // 13 SHOP1
	render.Etc = append(render.Etc, rl.NewRectangle(993, 156, 104, 104))  // 14 SHOP2
	render.Etc = append(render.Etc, rl.NewRectangle(1132, 228, 17, 17))   // 15 SNOW1
	render.Etc = append(render.Etc, rl.NewRectangle(1157, 229, 15, 15))   // 16 SNOW2
	render.Etc = append(render.Etc, rl.NewRectangle(1182, 228, 15, 15))   // 17 SNOW3
	render.Etc = append(render.Etc, rl.NewRectangle(254, 37, 16, 16))     // 18 TURRET
	render.Etc = append(render.Etc, rl.NewRectangle(274, 38, 14, 14))     // 19 NINJA STAR
	render.Etc = append(render.Etc, rl.NewRectangle(292, 36, 16, 16))     // 20 OIL BARREL
	render.Etc = append(render.Etc, rl.NewRectangle(344, 60, 15, 15))     // 21 SWITCH 1
	render.Etc = append(render.Etc, rl.NewRectangle(361, 60, 15, 15))     // 22 SWITCH 2
	render.Etc = append(render.Etc, rl.NewRectangle(445, 58, 17, 17))     // 23 CHEST
	render.Etc = append(render.Etc, rl.NewRectangle(717, 37, 12, 12))     // 24 SWITCH ARROW
	render.Etc = append(render.Etc, rl.NewRectangle(998, 414, 200, 200))  // 25 NIGHT REC
	render.Etc = append(render.Etc, rl.NewRectangle(731, 33, 18, 18))     // 26 SPINNING TRAP
	render.Etc = append(render.Etc, rl.NewRectangle(256, 16, 16, 16))     // 27 POWERUP BLOK
	render.Etc = append(render.Etc, rl.NewRectangle(820, 34, 18, 18))     // 28 MAP
	render.Etc = append(render.Etc, rl.NewRectangle(842, 32, 18, 18))     // 29 FIRE TRAIL
	render.Etc = append(render.Etc, rl.NewRectangle(863, 35, 16, 16))     // 30 INVISIBILITY
	render.Etc = append(render.Etc, rl.NewRectangle(883, 33, 17, 17))     // 31 COFFEE
	render.Etc = append(render.Etc, rl.NewRectangle(905, 34, 18, 18))     // 32 TELEPORT
	render.Etc = append(render.Etc, rl.NewRectangle(927, 35, 16, 16))     // 33 ATTACK RANGE
	render.Etc = append(render.Etc, rl.NewRectangle(945, 35, 16, 16))     // 34 ATTACK DAMAGE
	render.Etc = append(render.Etc, rl.NewRectangle(587, 58, 15, 15))     // 35 ORBITAL
	render.Etc = append(render.Etc, rl.NewRectangle(607, 58, 16, 16))     // 36 CHAIN LIGHTNING
	render.Etc = append(render.Etc, rl.NewRectangle(624, 57, 16, 16))     // 37 HP RING
	render.Etc = append(render.Etc, rl.NewRectangle(641, 57, 16, 16))     // 38 ARMOR SHIELD
	render.Etc = append(render.Etc, rl.NewRectangle(661, 56, 18, 18))     // 39 ANCHOR PAUSE ENEMY
	render.Etc = append(render.Etc, rl.NewRectangle(681, 55, 18, 18))     // 40 UMBRELLA RAIN
	render.Etc = append(render.Etc, rl.NewRectangle(705, 57, 14, 14))     // 41 DASH
	render.Etc = append(render.Etc, rl.NewRectangle(724, 56, 18, 18))     // 42 SOCKS POISON GAS
	render.Etc = append(render.Etc, rl.NewRectangle(748, 56, 17, 17))     // 43 CHERRIES
	render.Etc = append(render.Etc, rl.NewRectangle(770, 58, 15, 15))     // 44 FISH FLOOD
	render.Etc = append(render.Etc, rl.NewRectangle(789, 57, 18, 18))     // 45 CAKE BIRTHDAY
	render.Etc = append(render.Etc, rl.NewRectangle(811, 58, 16, 16))     // 46 PEACE NO DAMAGE
	render.Etc = append(render.Etc, rl.NewRectangle(831, 58, 16, 16))     // 47 ALIEN SPECIAL ENEMY
	render.Etc = append(render.Etc, rl.NewRectangle(850, 57, 16, 16))     // 48 AIR STRIKE
	render.Etc = append(render.Etc, rl.NewRectangle(893, 58, 14, 14))     // 49 POWERUP FIREWORKS
	render.Etc = append(render.Etc, rl.NewRectangle(909, 58, 14, 14))     // 50 CARROT COMPANION
	render.Etc = append(render.Etc, rl.NewRectangle(870, 55, 18, 18))     // 51 MARIO PLATFORMER LEVEL
	render.Etc = append(render.Etc, rl.NewRectangle(1132, 80, 64, 64))    // 52 FOOTPRINTS
	render.Etc = append(render.Etc, rl.NewRectangle(966, 35, 14, 14))     // 53 STAIRCASE
	render.Etc = append(render.Etc, rl.NewRectangle(41, 1219, 565, 300))  // 54 BITTY KNIGHT LOGO
	render.Etc = append(render.Etc, rl.NewRectangle(748, 1214, 308, 305)) // 55 GO LOGO
	render.Etc = append(render.Etc, rl.NewRectangle(10, 1532, 256, 256))  // 56 RAYLIB
	render.Etc = append(render.Etc, rl.NewRectangle(938, 104, 20, 20))    // 57 BOSS 3 PROJ
	render.Etc = append(render.Etc, rl.NewRectangle(841, 818, 128, 128))  // 58 POISON GAS

	render.Plants = append(render.Plants, rl.NewRectangle(315, 36, 13, 13))
	render.Plants = append(render.Plants, rl.NewRectangle(329, 38, 11, 11))
	render.Plants = append(render.Plants, rl.NewRectangle(341, 37, 12, 12))
	render.Plants = append(render.Plants, rl.NewRectangle(354, 31, 18, 18))
	render.Plants = append(render.Plants, rl.NewRectangle(373, 31, 18, 18))
	render.Plants = append(render.Plants, rl.NewRectangle(392, 33, 16, 16))
	render.Plants = append(render.Plants, rl.NewRectangle(409, 33, 16, 16))
	render.Plants = append(render.Plants, rl.NewRectangle(425, 33, 16, 16))
	render.Plants = append(render.Plants, rl.NewRectangle(442, 35, 14, 14))
	render.Plants = append(render.Plants, rl.NewRectangle(457, 35, 14, 14))
	...
	//FLOOR
	for {
		render.Floortiles = append(render.Floortiles, rl.NewRectangle(x, y, 16, 16))
		x += 16
		if x >= 896 {
			break
		}
	...
	y = 16
	for {
		render.Walltiles = append(render.Walltiles, rl.NewRectangle(x, y, 16, 16))
		x += 16
		if x >= 256 {
			break
		}
	...
	y = 351
	for {
		render.Bats = append(render.Bats, rl.NewRectangle(x, y, 16, 16))
		y += 16
		if y > 383 {
			break
		}
	...
	//KNIGHT MOVE
	x = 935
	player.Pl.imgWalkX = x
	y = 268
	for a := 0; a < 4; a++ {
		render.Knight = append(render.Knight, rl.NewRectangle(x, y, 32, 32))
		y += 32
	}
	//KNIGHT ATTACK
	x = 735
	...
	y = 34
	for a := 0; a < 7; a++ {
		render.Skulls = append(render.Skulls, rl.NewRectangle(x, y, 16, 16))
		x += 16
	}
	//CANDLES
	x = 586
	y = 34
	for a := 0; a < 3; a++ {
		render.Candles = append(render.Candles, rl.NewRectangle(x, y, 16, 16))
		x += 16
	}
	//SIGNS
	x = 634
	y = 34
	for a := 0; a < 5; a++ {
		render.Signs = append(render.Signs, rl.NewRectangle(x, y, 16, 16))
		x += 16
	}
	//SPLATS
	x = 0
	y = 1072
	for a := 0; a < 6; a++ {
		render.Splats = append(render.Splats, rl.NewRectangle(x, y, 128, 128))
		x += 128
	}
	//STATUES
	x = 0
	y = 175
	for a := 0; a < 16; a++ {
		render.Statues = append(render.Statues, rl.NewRectangle(x, y, 48, 48))
		x += 48
	}
	//MUSHROOMS
	x = 751
	y = 34
	for a := 0; a < 4; a++ {
		render.Mushrooms = append(render.Mushrooms, rl.NewRectangle(x, y, 16, 16))
		x += 16
	}
	//PATTERNS
	x = 2
	y = 798
	for a := 0; a < 6; a++ {
		render.Patterns = append(render.Patterns, rl.NewRectangle(x, y, 128, 128))
		render.Patterns = append(render.Patterns, rl.NewRectangle(x, y+128, 128, 128))
		x += 128
	}
	//GEMS
	x = 0
	y = 297
	for a := 0; a < 8; a++ {
		render.Gems = append(render.Gems, rl.NewRectangle(x, y, 32, 32))
		x += 32
	}

	//RABBIT1
	render.Rabbit1.frames = 3
	render.Rabbit1.xl = 68
	render.Rabbit1.yt = 335
	render.Rabbit1.W = 16
	render.Rabbit1.recTL = rl.NewRectangle(render.Rabbit1.xl, render.Rabbit1.yt, render.Rabbit1.W, render.Rabbit1.W)

	//FIREBALL PLAYER
	render.FireballPlayer.frames = 3
	render.FireballPlayer.xl = 1
	render.FireballPlayer.yt = 60
	render.FireballPlayer.W = 16
	render.FireballPlayer.recTL = rl.NewRectangle(render.FireballPlayer.xl, render.FireballPlayer.yt, render.FireballPlayer.W, render.FireballPlayer.W)

	//BURN
	render.Burn.frames = 3
	render.Burn.xl = 72
	render.Burn.yt = 60
	render.Burn.W = 16
	render.Burn.recTL = rl.NewRectangle(render.Burn.xl, render.Burn.yt, render.Burn.W, render.Burn.W)

	//STAR
	render.Star.frames = 3
	render.Star.xl = 144
	render.Star.yt = 60
	render.Star.W = 16
	render.Star.recTL = rl.NewRectangle(render.Star.xl, render.Star.yt, render.Star.W, render.Star.W)

	//WATER
	render.Wateranim.frames = 3
	render.Wateranim.xl = 208
	render.Wateranim.yt = 60
	render.Wateranim.W = 16
	render.Wateranim.recTL = rl.NewRectangle(render.Wateranim.xl, render.Wateranim.yt, render.Wateranim.W, render.Wateranim.W)

	//PLANT BULL
	render.PlantBull.frames = 4
	render.PlantBull.xl = 276
	render.PlantBull.yt = 60
	render.PlantBull.W = 16
	render.PlantBull.recTL = rl.NewRectangle(render.PlantBull.xl, render.PlantBull.yt, render.PlantBull.W, render.PlantBull.W)

	//SPIKES
	render.Spikes.frames = 13
	render.Spikes.xl = 0
	render.Spikes.yt = 90
	render.Spikes.W = 32
	render.Spikes.recTL = rl.NewRectangle(render.Spikes.xl, render.Spikes.yt, render.Spikes.W, render.Spikes.W)

	//SPRING
	render.Spring.frames = 2
	render.Spring.xl = 382
	render.Spring.yt = 59
	render.Spring.W = 16
	render.Spring.recTL = rl.NewRectangle(render.Spring.xl, render.Spring.yt, render.Spring.W, render.Spring.W)

	//POISON GAS
	render.Posiongas.frames = 3
	render.Posiongas.xl = 686
	render.Posiongas.yt = 638
	render.Posiongas.W = 128
	render.Posiongas.recTL = rl.NewRectangle(render.Posiongas.xl, render.Posiongas.yt, render.Posiongas.W, render.Posiongas.W)

	//MUSHROOM BULL
	render.MushBull.frames = 3
	render.MushBull.xl = 516
	render.MushBull.yt = 59
	render.MushBull.W = 16
	render.MushBull.recTL = rl.NewRectangle(render.MushBull.xl, render.MushBull.yt, render.MushBull.W, render.MushBull.W)

	//BLADES
	render.Blades.frames = 7
	render.Blades.xl = 456
	render.Blades.yt = 92
	render.Blades.W = 32
	render.Blades.recTL = rl.NewRectangle(render.Blades.xl, render.Blades.yt, render.Blades.W, render.Blades.W)

	//SPEAR
	render.Spear.frames = 7
	render.Spear.xl = 1004
	render.Spear.yt = 5
	render.Spear.W = 16
	render.Spear.recTL = rl.NewRectangle(render.Spear.xl, render.Spear.yt, render.Spear.W, 64)

	//FIRETRAIL
	render.Firetrailanim.frames = 3
	render.Firetrailanim.xl = 724
	render.Firetrailanim.yt = 82
	render.Firetrailanim.W = 16
	render.Firetrailanim.recTL = rl.NewRectangle(render.Firetrailanim.xl, render.Firetrailanim.yt, render.Firetrailanim.W, 16)

	//ORBITAL
	render.Orbitalanim.frames = 5
	render.Orbitalanim.xl = 800
	render.Orbitalanim.yt = 84
	render.Orbitalanim.W = 16
	render.Orbitalanim.recTL = rl.NewRectangle(render.Orbitalanim.xl, render.Orbitalanim.yt, render.Orbitalanim.W, 16)

	//FLOOD
	render.Floodanim.frames = 3
	render.Floodanim.xl = 900
	render.Floodanim.yt = 82
	render.Floodanim.W = 16
	render.Floodanim.recTL = rl.NewRectangle(render.Floodanim.xl, render.Floodanim.yt, render.Floodanim.W, 16)
	fx.FloodImg = render.Floodanim.recTL

	//FISH R
	render.FishR.frames = 3
	render.FishR.xl = 724
	render.FishR.yt = 105
	render.FishR.W = 16
	render.FishR.recTL = rl.NewRectangle(render.FishR.xl, render.FishR.yt, render.FishR.W, 16)

	//FISH L
	render.FishL.frames = 3
	render.FishL.xl = 794
	render.FishL.yt = 105
	render.FishL.W = 16
	render.FishL.recTL = rl.NewRectangle(render.FishL.xl, render.FishL.yt, render.FishL.W, 16)

	//AIRSTRIKE
	render.Airstrikeanim.frames = 3
	render.Airstrikeanim.xl = 664
	render.Airstrikeanim.yt = 614
	render.Airstrikeanim.W = 32
	render.Airstrikeanim.recTL = rl.NewRectangle(render.Airstrikeanim.xl, render.Airstrikeanim.yt, render.Airstrikeanim.W, 32)

	//BOSS1 PROJ
	render.Boss1anim.frames = 3
	render.Boss1anim.xl = 977
	render.Boss1anim.yt = 83
	render.Boss1anim.W = 16
	render.Boss1anim.recTL = rl.NewRectangle(render.Boss1anim.xl, render.Boss1anim.yt, render.Boss1anim.W, 16)

	//BOSS2 PROJ
	render.Boss2anim.frames = 3
	render.Boss2anim.xl = 866
	render.Boss2anim.yt = 107
	render.Boss2anim.W = 16
	render.Boss2anim.recTL = rl.NewRectangle(render.Boss2anim.xl, render.Boss2anim.yt, render.Boss2anim.W, 16)

	//ALIEN
	x = 1008
	y = 770
	for {
		render.Alien = append(render.Alien, rl.NewRectangle(x, y, 64, 64))
		x += 64
		if x >= 1200 {
			x = 1008
			y += 64
...
func inp(gs *GameState) { //MARK:INP
...
func timers(shop *ShopState, level *LevelState, core *CoreState) { //MARK:TIMERS
  if shop.ShopExitT > 0 {
    shop.ShopExitT--
  }
  level.RunT++
  if level.RunT%core.Fps == 0 {
    level.Secs++
    level.RunT = 0
  }
  if level.Secs == 60 {
  ...
  if level.RoomChangedTimer > 0 {
    level.RoomChangedTimer--
    if level.RoomChangedTimer == 1 {
      level.RoomChanged = false
    }
  }
}
func cams(cam2 *rl.Camera2D, core CoreState, ui UIState, lvl LevelState) { //MARK:CAMS

  cam2.Target = core.Cnt

  cam2.Offset.X = core.ScrWF32 / 2
  cam2.Offset.Y = core.ScrHF32 / 2

  if lvl.Flipcam && !ui.OptionsOn {
    cam2.Rotation = 180
  } else if lvl.Flipcam && ui.OptionsOn {
    cam2.Rotation = 0
  } else {
    cam2.Rotation = 0
  }
...
func initialWindow(gs *GameState) { //MARK:INITIAL WINDOW
  ...
  gs.UI.HpBarsOn = true
  makeSettings(&gs.UI, &gs.Render, &gs.Player, &gs.Input, &gs.Audio ,&gs.Level)
  makeShaders(&gs.Render, gs.Core)
  makeAudio(&gs.Audio)
  makeFxInitial(&gs.FX, &gs.Core)
  makeImgs(&gs.Render, &gs.FX, &gs.Player)
  makeBosses(&gs.Core, &gs.Level)
  makelevel(&gs.Level, &gs.Shop, &gs.Enemies, gs.Core, gs.Render)
  makeCompanions(&gs.Companions, &gs.Core, &gs.Render)
  makePlayer(&gs.Player, &gs.Render, &gs.Core)
  makeTimes(&gs.Timing)
  cams(&gs.Render.Cam2, gs.Core, gs.UI, gs.Level)
...
func unload(render RenderState, audio AudioState) {
  rl.UnloadShader(render.Shader)
  rl.UnloadShader(render.Shader2)
  rl.UnloadRenderTexture(render.RenderTarget)
  rl.UnloadTexture(render.Imgs)

  rl.StopMusicStream(audio.Music)
  rl.UnloadMusicStream(audio.Music)
  rl.UnloadMusicStream(audio.BackMusic[audio.BgMusicNum])

  for a := 0; a < len(audio.Sfx); a++ {
    rl.UnloadSound(audio.Sfx[a])
  }
  rl.CloseAudioDevice()
...
func main() { //MARK:MAIN
  ...
  initialWindow(gs) //INITIAL INSIDE WINDOW
  ...
  for !rl.WindowShouldClose() {
    ...
    drawnoRender(gs.Player, gs.Level, gs.Core, gs.Render)
  ...
  unload(gs.Render, gs.Audio) //UNLOAD
